...
Risky-bash interceptor + step-up MFA audit MCP server, packaged as plugins for **Claude Code** and **OpenAI Codex CLI**. Both plugins share a single MCP server core and gate logic via npm workspaces — adding a third host (Cursor, Antigravity, …) means writing a new adapter + thin plugin shell, not duplicating the gate.
...
CI(`.github/workflows/ci.yml`)는 PR마다 `build:plugin` 실행 후 ① `packages/*/dist/`, ② `plugins/claude-code-ai-action-tracker/dist/`, ③ `plugins/codex-ai-action-tracker/dist/` 세 곳에 대해 `git diff --exit-code`로 빌드 산출물 동기성을 검증한다. hook smoke test 10종(claude-code 7 + codex 3)도 함께 통과해야 한다.
...
- Source 수정 후 반드시 `npm run build:plugin`을 거쳐 ① `packages/*/dist/`, ② `plugins/claude-code-ai-action-tracker/dist/`, ③ `plugins/codex-ai-action-tracker/dist/` 세 곳을 commit과 함께 동기화. CI가 모두 검증. dist/를 직접 편집하지 말 것 — 다음 빌드에서 덮어쓰여진다.
