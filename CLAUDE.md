...
- **New backend MCP tools are one definition object** in `packages/gate-backend/src/mcp-tools/*.ts` (step-up coordinate included via `stepUp`; wired by the `registerBackendTools()` loop). After adding one, run `npm run prebuild:plugin` — registration, docs, `GUARD_TOOL_NAMES`, and the CLI catalog all follow from the definition. → [.claude/rules/policy-and-state.md](.claude/rules/policy-and-state.md)
