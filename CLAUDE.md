...
/                                                # 루트 = marketplace + monorepo orchestrator
  ...
  .github/workflows/ci.yml                       # multi-plugin dist sync + 23종 hook smoke test.
...
- **Runtime kill-switch** (전역 enable/disable): `~/.transcodes/config.json`의 `enabled` 플래그 (token-store.ts가 관리. plugin 상태 파일이 모이는 `~/.transcodes/state/`와 같은 제품 홈 아래지만 `state/`와는 분리된 CLI 소유 파일 — CLI 프로세스와 4개 호스트 hook이 공유하는 고정 경로). `evaluatePreToolUse` 최상단 `isTrackerEnabled()` 체크가 비활성 시 `{kind:"pass"}` 반환 → Bash + 보호 MCP-tool 차단을 한 지점에서 동시 무력화. SessionStart primer(4개 host entry)도 별도 가드. **부재·손상 = 활성(true)** 기본(보안 게이트가 조용히 꺼지지 않도록).
  - **enable/disable 비대칭 (보안 핵심):** disable은 보호 약화이므로 **사람의 out-of-band 행동만** 허용 — 에이전트가 자기 가드레일을 끄지 못하게 함. ① MCP tool `set_tracker_enabled`는 `enabled=true`만 허용하고 `false`는 거부(`get_tracker_status`는 read-only 조회). ② 에이전트의 Bash `transcodes disable` 시도는 system 패턴 `tracker-self-disable`이 step-up으로 차단. ③ 사람이 직접 친 터미널 `transcodes disable`만 게이트를 끔(hook 부재). ④ GUI dashboard(`transcodes` 무인자)의 `POST /api/settings`도 disable 경로지만, 에이전트의 dashboard 기동을 막던 system 패턴 `tracker-dashboard-launch`는 **제거됨** — 이 패턴은 명령 문자열 전체에서 단어 `transcodes`의 단순 출현(주석·인용 인자·`transcodes-guard` 패키지명·리포 디렉토리명)을 잡아 오탐이 만연했고, leading lookbehind `(?<![/.\w])` 화이트리스트 방식으로는 근본 해소가 불가했다. dashboard 서버(`dashboard.ts`)는 Host 헤더 검증으로 DNS rebinding을 거부한다. **알려진 한계**: 패턴 제거로 에이전트의 dashboard 기동(→ `POST /api/settings`로 disable)과 사람이 이미 띄운 dashboard에 `curl POST /api/settings {enabled:false}` 양쪽 모두 danger-pattern으로는 미차단(localhost HTTP는 발신자 식별 불가). disable의 권위적 방어는 ②의 `tracker-self-disable`(명시적 `transcodes disable` 차단)과 MCP `set_tracker_enabled`의 `false` 거부에 남으며, dashboard 토글 완전 차단은 토글 자체에 WebAuthn step-up 게이트가 필요(미구현). enable은 보호 강화라 에이전트가 해도 안전 → MCP·CLI 양쪽 자유. Claude Code 사용자는 네이티브 `/plugin disable`로 hook+MCP 완전 언로드도 가능.
...
- 영속/캐시 디렉토리 해석은 항상 `@ai-action-tracker/plugin-paths`의 `dataDir()` / `cacheDir()`만 사용. `os.homedir()` 직접 join이나 경로 하드코딩 금지. **둘 다 host 무관하게 `~/.transcodes/state/`로 해석**된다(CLI가 `~/.transcodes/config.json`을 두는 제품 홈의 `state/` 하위 — 모든 로컬 상태가 한 곳). `dataDir`/`cacheDir`는 의미 구분용으로 분리돼 있을 뿐 현재 같은 경로. 이전의 host별 분기(`CLAUDE_PLUGIN_DATA` / `~/.claude/ai-action-tracker/` / OS 캐시)는 폐기되고 `migrateLegacyFile`의 **마이그레이션 소스로만** 잔존. 새 영속 파일을 추가할 때 `migrateLegacyFile(name, kind)`를 첫 read 진입점에서 호출해 세 옛 위치에서 기존 사용자 데이터를 자동 마이그레이션.
...
- **릴리스 자동화는 release-please 단일 루트 컴포넌트 모델로만 — 단, 현재 실제 배포(publish)는 보류 상태.** `.github/workflows/release.yml`은 release-please **자동화만** 수행한다: main의 conventional commit → Release PR → 머지 시 ① 버전 bump ② 루트 CHANGELOG ③ git tag(`transcodes-guard-vX.Y.Z`). **npm publish 등 발행 step은 없다** — 배포 채널을 아직 확정하지 않았기 때문(아래). 4개 plugin `package.json`·매니페스트 version은 `release-please-config.json`의 `extra-files`로 **동일 버전 함께** 자동 동기되고 버전 진실원천은 루트 `package.json` + `.release-please-manifest.json` 하나 — **수동 bump 금지**(트레인 정합성). 즉 기능 개발 동안 버전·CHANGELOG·tag 기록만 유지되고, 배포 결정이 서면 그때 publish step/별도 workflow를 추가한다.
  - **배포 채널 (미확정, 호스트별로 다름 — 조사·계획은 [`docs/research/multi-host-plugin-distribution.md`](./docs/research/multi-host-plugin-distribution.md)):** npm은 **Claude Code의 선택적 채널**일 뿐이다(marketplace.json의 `"source":"npm"`). Claude Code 정식 경로는 **marketplace + git source**(이 repo 루트가 marketplace, plugin을 상대경로로 참조 → 사용자는 `/plugin marketplace add <owner/repo>` + `/plugin install`; dist가 repo에 커밋돼 있어 클론 즉시 설치). Codex/Antigravity/Cursor는 **npm이 배포 채널이 아니다** — 각각 Codex 네이티브 marketplace(`.agents/plugins/marketplace.json` + git-subdir), `agy plugin install <git-url>`, `install.sh`(로컬 JSON config). 그래서 plugin마다 npm 패키지를 따로 만들 필요가 없으며, npm 발행 여부는 Claude Code 한정으로 추후 결정한다.
  - plugin은 tsup `noExternal`로 `@ai-action-tracker/*`를 번들해 self-contained 빌드되므로 내부 패키지는 private 유지(별도 발행 금지). `@bigstrider/transcodes-cli`는 버전 동기 대상이 아니며 plugin의 `peerDependencies` 범위로만 호환을 선언. plugin `package.json`의 `publishConfig`/패키지명은 향후 npm 채널 대비 잔존하나, 현재 어떤 plugin도 발행되지 않는다.
