...
- fast-path verified consume diverges by **rule** (not kind): controlled by the `consume_in_hook` field on each tool-rule.
  - **Bash**: always consumed in the hook (no follow-up handler).
  - **MCP system rules** (`consume_in_hook=false`, default for `hooks/tool-rules.json`): defer consume to the tool handler via `withStepupVerifiedSid` — the handler needs the sid to set `X-Step-Up-Session-Id` on the backend call before discarding the record.
  - **MCP user rules** (`consume_in_hook=true`, default for `add_tool_rule`): consumed in the hook so handlers without the `withStepupVerifiedSid` wrapper still satisfy the single-shot policy. Set `consume_in_hook=false` explicitly only when the user-registered tool's handler wraps with `withStepupVerifiedSid` and needs to thread the sid downstream.
- Stop hook orphan reap: when Stop fires, the turn is done — any `verified.json` still present cannot be "in flight." Stop silently `consumeVerified()`+`clearPending()` two orphan patterns (verified present + pending absent or `status="verified"`; verified absent + pending `status="verified"`). This is the backstop for system-rule deferred consume when the handler throws before `withStepupVerifiedSid`, and prevents false "dangling pending" reminders.
- Known limit — **concurrent MCP race**: `verified.json` is a single file with no inter-process locking. Two parallel hook invocations for system rules (`consume_in_hook=false`) can both pass `readVerified()` and reach handlers with the same sid, producing two backend calls with the same `X-Step-Up-Session-Id`. Authoritative backstop is Transcodes backend sid-replay rejection. No client-side fix planned. If a specific tool cannot tolerate this, re-register its rule with `consume_in_hook=true` (loses the ability to thread sid to backend).
