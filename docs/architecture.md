...
위 4종(PreToolUse + SessionStart + UserPromptSubmit + Stop)은 Claude Code의 hook event 집합이다. Codex는 1:1로 같은 wire format을 채택했고, **Antigravity 2.0는 다른 hook event 집합 + 다른 wire format**을 사용한다.
