...
```
        ...
        │ (Bash)     │   │  Submit   │  │         │   │            │
        └────────────┘   └───────────┘  └─────────┘   └────────────┘
        deny + system    additional      additional    additional
        Message          Context         Context       Context (primer)
```

| Hook | 트리거 | 채널 | 역할 |
|------|-------|-----|------|
| `PreToolUse` (Bash) | 매 Bash 호출 | stdout JSON `permissionDecision: "deny"` + `systemMessage` | 위험 패턴 매치 시 step-up 세션 생성, pending 파일 write, **명령 차단**. 신속 경로(verified 소비) 시 양 파일 모두 정리 후 통과. |
| `SessionStart` | 세션 시작·resume·clear·compact | `additionalContext` | 프로토콜 1회 사전 주입. 이월된 pending이 있으면 sid·URL 함께 노출. |
| `UserPromptSubmit` | 매 사용자 메시지 | `additionalContext` | 사용자 메시지에 "완료" 류 키워드 + pending 존재 → polling 재개 신호. |
| `Stop` | AI 응답 종료 직전 | `additionalContext` | dangling pending(status=`pending`) 또는 unconsumed verified 감지 시 reminder. `decision: "block"` 절대 미사용. |
