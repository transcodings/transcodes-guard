...
```
       packages/mcp-server-core/src/server.ts
     createServer() → McpServer
              │
       ┌──────┴───────┐
       ↓              ↓
  plugins/*/src/   plugins/claude-code/src/
     stdio.ts          http.ts
  (로컬 진입)        (원격 진입)
```

`packages/mcp-server-core/src/server.ts`는 **transport를 모른다.** 도구·리소스·프롬프트만 정의하고 `McpServer` 인스턴스를 반환한다. 각 플러그인의 `src/stdio.ts`·`src/http.ts`는 적합한 `*Transport`를 만들고 `server.connect(transport)` 한 줄로 연결한다.
...
`plugins/claude-code/src/http.ts`에는 인증 로직이 없다. **로컬 개발에는 OK, 비-루프백 배포에는 절대 안 된다.**
...
자세한 절차·검증은 `docs/prd/`(필요 시) 또는 `packages/stepup-core/src/` 코드 헤더 docstring 참조. 새 민감 기능은 이 orchestra를 소비하고, hook을 추가로 늘리지 않는 것을 권장.
...
### 다중 호스트 hook orchestra 변형

위 4종(PreToolUse + SessionStart + UserPromptSubmit + Stop)은 Claude Code의 hook event 집합이다. Codex는 1:1로 같은 wire format을 채택했고, **Antigravity 2.0**는 다른 hook event 집합 + 다른 wire format, **Cursor**는 다른 wire format(평면)을 사용한다.

| 호스트 | Hook 구성 | wire format 특이사항 |
|---|---|---|
| Claude Code | PreToolUse + SessionStart + UserPromptSubmit + Stop (**4종**) | 기본 형태. PreToolUse는 `hookSpecificOutput.permissionDecision`, SessionStart/UserPromptSubmit은 `hookSpecificOutput.additionalContext`, Stop은 top-level `decision: "block"`. stdin은 snake_case (`tool_name`/`tool_input`/`cwd`). |
| Codex CLI | 동일 4종 | `codexAdapter`가 `claudeCodeAdapter`에 위임. byte-level 동일 (legacy fallback alias 약간 다름). |
| Antigravity 2.0 | **PreToolUse + PreInvocation + Stop (3종)** | SessionStart/UserPromptSubmit 등가물 부재 → `PreInvocation`이 둘 다 담당: 최초 invocation(`invocationNum <= 1`, missing/non-numeric field 시 `0`으로 default해 defensive하게 fire)에서 primer 주입(SessionStart 등가) + 매 invocation에서 `transcript.jsonl` tail로 user "auth done" 감지(UserPromptSubmit 등가). wire format은 **native**: stdin은 camelCase + 중첩 `toolCall.name`/`toolCall.args`/`conversationId`/`workspacePaths`/`transcriptPath`. PreToolUse stdout은 **top-level** `decision: "allow"\|"deny"\|"ask"\|"force_ask"` + `reason` + `permissionOverrides?` (`hookSpecificOutput` wrapper 없음). PreInvocation stdout은 `{ injectSteps: [{ ephemeralMessage }] }`. Stop은 `decision: "continue"` + `reason` (Claude Code의 `block`과 verb 반전 — `continue`가 turn 종료를 막고 reason을 system message로 inject). shell tool 이름은 `Bash` 대신 **`run_command`** + args.command 대신 **args.CommandLine**. |
| Cursor IDE | **beforeShellExecution + beforeMCPExecution + sessionStart + beforeSubmitPrompt + stop (5종)** | PreToolUse를 event-specific으로 분리(matcher `MCP:plugin_transcodes-guard_*`). stdin은 Claude Code와 같은 snake_case (`tool_name`/`tool_input`/`cwd`) → 파싱 위임. PreToolUse stdout은 **평면** `{ permission: "allow"\|"deny", user_message?, agent_message?, updated_input? }` — `hookSpecificOutput` wrapper 없음. sessionStart는 평면 `{ additional_context }`. **beforeSubmitPrompt는 `{ continue, user_message? }`만 가능** — `additional_context` 채널 부재. 따라서 UserPromptSubmit 등가 hook은 context 주입 대신 **side effect로만**(consumeVerified + clearPending) 동작하고 항상 `{ continue: true }` 반환. Stop은 `{ followup_message }` (Claude Code `decision: "block"`과 의미 동일, 키 이름 차이). shell tool 이름은 **`Shell`**. |

새 host 추가 시 (1) `packages/hook-adapters/src/<host>.ts` 신설 → wire format 변환, (2) `plugins/<host>/` 매니페스트 + thin hook entry → host event 집합 매핑, 두 가지만으로 끝낸다. `packages/stepup-core/`와 `packages/mcp-server-core/`는 절대 host-aware 코드를 받지 않는다 — 단 하나의 예외는 `evaluate.ts:classifyToolCall`이 `"Bash"`/`"run_command"`/`"Shell"` 셋 다 인식하는 한 줄(shell tool 이름이 host별로 다른 것은 wire format 차이가 아니라 host의 단어 선택 차이이므로 host-neutral 분류기에 흡수). Antigravity adapter는 `args.CommandLine → args.command` normalize도 담당해 `classifyToolCall`이 host 무관 유지.

본 plugin의 1차 출시 unknown 4종(MCP tool naming convention / `${CLAUDE_PLUGIN_ROOT}` 등가 변수 / subagent의 hook stdin `conversationId` / Stop `continue` UX)은 [`research/antigravity-e2e-findings.md`](./research/antigravity-e2e-findings.md)에 추적. 모두 antigravity 바이너리로 1회 실측 후 narrow하면 되는 unknowns이며, 1차 코드는 광범위 fallback + 보수적 default로 작성됐다.
