...
| Hook | 트리거 | 채널 | 역할 |
|------|-------|-----|------|
| `PreToolUse` (Bash) | 매 Bash 호출 | stdout JSON `hookSpecificOutput.permissionDecision: "deny" \| "allow"` + `systemMessage` | 위험 패턴 매치 시 step-up 세션 생성, pending 파일 write, **명령 차단**. 신속 경로(verified 소비) 시 양 파일 정리 후 **명시적 `"allow"` JSON emit** — exit 0만으로는 settings.json deny 규칙·내장 safety를 override 못 함. |
...
| `Stop` | AI 응답 종료 직전 | top-level `{ decision: "block", reason }` (Stop은 `hookSpecificOutput.hookEventName` enum에 없어 v2 wrapper 사용 시 validator reject) | dangling pending(status=`pending`) 또는 unconsumed verified 감지 시 reminder. `reason` 텍스트가 다음 턴 컨텍스트로 surface. |
