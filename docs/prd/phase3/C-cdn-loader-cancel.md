...
> ~~규모: **L** · 선행: ~~A~~(완료, PR #44), [B](./B-cdn-deploy-cancel.md) · 외부 의존: **설계 결정(OQ1/3/4)** · 상태: ⚠️ Blocked (결정 선행)~~
...
`backend.ts`·`registry.ts`의 "Phase 2 (CDN)" 번호를 phase3 기준으로 정정(부모 §1, ~~Unit E~~(완료, PR #48)와 함께).
