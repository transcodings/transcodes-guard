...
- **G2** — 결정 권한의 잔여 갭을 봉합한다(좌표 불일치 fail-closed, 결정 감사 로그). → [H](./phase3/H-server-decision-done.md)
...
|---|---|:--:|---|---|---|
| [A](./phase3/A-obfuscate-build-done.md) | ~~난독화 빌드~~ | M | — | — | 🎉 Done (#44) · **산출물 보류** |
...
| [E](./phase3/E-lint-promotion-done.md) | 경계 lint error 승격 | S | — | 없음 | 🎉 **Done** (#48) |
...
| [H](./phase3/H-server-decision-done.md) | **결정 권한 봉합** | **S** | — | ~~감사 API~~ 없음(기존 `/v1/audit/logs` 재사용) | 🎉 **Done** (#54) |
...
5. **D5 — 번들 스코프 org vs project** (신설, 2026-06-10 백엔드 탐사) → affects **G**. 권고: **project** — 백엔드 테넌트 패턴(`_project_id` 필터, RBAC role/resource의 project 스코프)과 정합. **백엔드에 만들어야 할 것의 단일 명세는 G 문서 §백엔드 구현 명세.**
...
| **M7 — 정책 번들** | G | 번들 API 계약 + fetch/검증/캐시 + fail-closed + 23종 smoke 갱신 | L | 클라이언트 Done(#52·#58·#60)·백엔드 잔여 |
