...
> Status: Draft v2 · Owner: huskyhoochu · Last updated: 2026-06-12 — 완료 단위 문서(A·E·H)는 제거됨(내역은 PR #44/#48/#54), 잔여 작업의 단일 목록은 [`phase3/remaining-work.md`](./phase3/remaining-work.md)
...
- ~~**G2** — 결정 권한의 잔여 갭을 봉합한다(좌표 불일치 fail-closed, 결정 감사 로그).~~ → ~~H~~(완료, PR #54)
...
|---|---|:--:|---|---|---|
| ~~A~~ | ~~난독화 빌드~~ | ~~M~~ | — | — | ~~🎉 Done (#44) · 산출물 보류~~ |
| [B](./phase3/B-cdn-deploy-cancel.md) | ~~CDN 배포~~ | — | — | — | 🗑 **Superseded** → G |
| [C](./phase3/C-cdn-loader-cancel.md) | ~~CDN 로더~~ | — | — | — | 🗑 **Superseded** → G |
| [D](./phase3/D-mirror-automation-cancel.md) | ~~미러 자동화~~ | — | — | — | 🗑 **Superseded** → J |
| ~~E~~ | ~~경계 lint error 승격~~ | ~~S~~ | — | — | ~~🎉 Done (#48)~~ |
| [F](./phase3/F-public-release-cancel.md) | ~~미러 공개~~ | — | — | — | 🗑 **Superseded** → J |
| [G](./phase3/G-policy-bundle.md) | **정책 번들 분리** | **L** | — | 백엔드 룰 저장소+CRUD+번들 뷰(사내) | 🔧 **In progress** (클라이언트 완료: G1 #52 · G2 #58 · G3 #60 — 잔여: dist baseline 처분(D2)+백엔드) |
| ~~H~~ | ~~결정 권한 봉합~~ | ~~S~~ | — | — | ~~🎉 Done (#54)~~ |
...
**권장 실행 순서: ~~G·H(병렬 가능)~~ → I(D1 결정 후) → J.** G 클라이언트 측·H 완료 — 잔여는 [`phase3/remaining-work.md`](./phase3/remaining-work.md). v1과 달리 외부 인프라(AWS·도메인) 의존이 0이고, 유일한 외부 의존은 자사 백엔드 API다.
...
2. ~~**D2 — 시스템 tool-rules의 기밀성**~~ ✅ 결정(2026-06-12): **비밀 아님** + 사용자 임의 수정 방지 단서 — G 문서 §5.
3. ~~**D3 — 번들 무결성 방식**~~ ✅ 결정: **TLS + SHA-384 manifest** (detached signature는 실위협 시).
4. ~~**D4 — 히스토리 처분**~~ ✅ 해소(2026-06-12): 스캔 2종+수동 감사 클린 → **직접 공개** (J 문서 D4 절).
5. ~~**D5 — 번들 스코프 org vs project**~~ ✅ 결정(2026-06-12): **project** — 클라이언트 캐시 키 반영 완료(PR #65). 백엔드 명세는 [`phase3/remaining-work.md`](./phase3/remaining-work.md).
...
| **M7 — 정책 번들** | G | ~~번들 클라이언트(fetch/검증/캐시/병합/smoke)~~ 완료 — 백엔드 잔여 | L | ~~클라이언트 Done(#52·#58·#60·#65·#67·#70)~~·백엔드 잔여 |
| ~~M8 — 결정 봉합~~ | ~~H~~ | ~~rbac-check fail-closed + 결정 감사~~ | ~~S~~ | ~~Done (#54)~~ |
