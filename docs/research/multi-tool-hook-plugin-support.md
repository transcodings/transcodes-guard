...
> 날짜: 2026-05-25 (Asia/Seoul) | 키워드: Codex CLI / Google Antigravity 2.0 / Cursor — hook · MCP · plugin 매니페스트 비교, transcodes-guard 다중 호스트 포팅 전략
>
> **개정 이력**
> - **2026-05-27 v4 (Cursor plugin 구현 확정)** — `plugins/cursor-ai-action-tracker/` 신설 + `cursorAdapter` 추가. Cursor 공식 docs `cursor.com/docs/agent/hooks` 더블체크로 v1 가정 모두 확인: ① PreToolUse 평면 `{ permission, user_message, agent_message, updated_input }` 출력 (no `hookSpecificOutput` wrapper), ② stdin은 Claude Code와 동일 snake_case → parse는 `claudeCodeAdapter`에 위임, ③ Stop은 `{ followup_message }` (Claude Code `decision:"block"`과 의미 같음, 키 이름만 다름), ④ **beforeSubmitPrompt는 `additional_context` 채널 부재** → UserPromptSubmit 등가 hook은 side effect (`consumeVerified` + `clearPending`)만 수행하고 항상 `{ continue: true }` 반환. 본 v4 진행 사항은 코드/CI/CLAUDE.md/architecture.md §5에 모두 반영. 본문은 v3 그대로 보존.
> - **2026-05-26 v3 (Antigravity 2.0 spec 정정 — Claude Code 호환 가정 폐기)** — Antigravity plugin 구현 plan 작성 단계에서 google-dev-knowledge MCP로 `antigravity.google/docs/{hooks,plugins,mcp,subagents,features,gcli-migration,home,overview}` 8종을 직접 fetch한 결과, v2의 핵심 결론 "Claude Code와 1:1 동일"이 **공식 spec과 정면 충돌**. v2가 의존한 lobehub 비공식 skill(현재 HTTP 403으로 검증 불가)이 잘못된 representation으로 판명. 정정 사항: ① hooks.json wrapper는 `{"<hook-group>": {"PreToolUse": [...]}}` (Claude Code의 단일 `hooks` wrapper 아님), ② PreToolUse stdin은 `toolCall.name`/`toolCall.args`/`stepIdx`/`conversationId`/`workspacePaths`/`transcriptPath`/`artifactDirectoryPath` (camelCase + nested toolCall — Claude Code snake_case와 완전 다름), ③ PreToolUse stdout은 **top-level** `decision: "allow"|"deny"|"ask"|"force_ask"` + `reason` + `permissionOverrides?` (Claude Code의 `hookSpecificOutput.permissionDecision` 아님), ④ Hook events는 **PreToolUse/PostToolUse/PreInvocation/PostInvocation/Stop 5종만** (SessionStart/UserPromptSubmit/PreCompact/SubagentStop 등 부재 — v2의 9종 주장 오류), ⑤ Tool name은 `Bash` 아닌 `run_command` (file edit은 `write_to_file`/`replace_file_content`/`multi_replace_file_content`), ⑥ `${CLAUDE_PLUGIN_ROOT}` 등가 변수 spec에 명시 없음, ⑦ Stop hook은 `decision: "continue"`로 turn 종료 차단(Claude Code의 `block`과 의미 반전 가능), ⑧ Antigravity hook 계보는 Claude Code가 아닌 **Gemini CLI**로부터 incorporate(공식 마이그레이션 docs 명시), ⑨ corpus가 2.0 시점 snapshot임은 `docs/features`의 "Antigravity 2.0 Features → JSON Hooks"가 `/docs/hooks`를 직접 link하는 cross-reference로 확정. 결과: codex 같은 claudeCodeAdapter 위임 패턴 적용 불가 → native antigravity adapter 작성 필수. 본 리포지토리의 [`antigravity-e2e-findings.md`](./antigravity-e2e-findings.md)에 implementation 전 수동 e2e 검증 슬롯 4종(MCP tool naming / plugin-bundled MCP load / subagent hook stdin / Stop `continue` UX) 명시.
> - **2026-05-25 v2 (Antigravity 2.0 refresh)** — 2026-05-19 Google I/O 키노트에서 발표된 Antigravity 2.0 (standalone desktop app + CLI + SDK + Managed Agents)을 기준으로 Antigravity 관련 모든 섹션을 재작성. 핵심 변경: ① Antigravity의 hook 설정 파일 형식이 Claude Code와 1:1 동일 확인(`${CLAUDE_PLUGIN_ROOT}` 변수 그대로 재사용), ② before/after model call 등 2종 신규 hook event 발견, ③ Dynamic Subagents는 static + 런타임 둘 다 가능, ④ Antigravity 포팅 난이도 **상 → 중** 하향, ⑤ 1.0 → 2.0 backwards-compat break 3건 추가. ⚠️ v3에서 ①②⑤ 결론이 **거짓으로 판명** — lobehub 비공식 skill 의존이 원인. 본 v2 본문의 핵심 발견사항 표는 historical record로 보존하되, v3 정정을 우선 적용해야 한다.
...
| 7 | 모든 도구가 **plugin 내부에서 hook + MCP server + skills/rules + slash commands**를 함께 번들링하는 모델을 공유 → transcodes-guard 한 패키지 안에 모두 담는 현재 구조는 그대로 유지 가능 | 4 공식문서 | 높음 |
...
| 9 | Cursor에는 `failClosed: true` 플래그가 있어 hook 실패 시 deny로 fallback 가능 — 현재 transcodes-guard의 "fail-open before / fail-safe after" 정책과 정렬 가능 | cursor.com/docs/hooks | 높음 |
...
├── config.toml                  # [mcp_servers.transcodes-guard]
├── hooks/
...
├── plugin.json                        # { "name": "transcodes-guard", "description": "..." }
├── mcp_config.json                    # { mcpServers: { "transcodes-guard": { command, args } } }
├── hooks/
...
│   │   ├── package.json                      # "@transcodes-guard/stepup-core"
│   │   └── tsconfig.json
...
│   │   └── package.json                      # "@transcodes-guard/hook-adapters"
...
│   │   └── package.json                      # "@transcodes-guard/mcp-server-core"
...
└── plugins/                                  # 호스트별 얇은 어댑터
    ...
    │   └── package.json                      # deps: @transcodes-guard/{stepup-core,hook-adapters,mcp-server-core}
...
```ts
import type { GateDecision } from "@transcodes-guard/stepup-core";
...
import { evaluateBashCommand, writePending } from "@transcodes-guard/stepup-core";
...
3. `failClosed: true` 옵션을 모든 step-up hook에 설정 → 현재 transcodes-guard의 fail-safe 정책과 일치.
4. `beforeMCPExecution`을 별도 등록하여 우리 MCP tool 자체 호출도 게이트 (현재 `Bash|mcp__plugin_transcodes-guard_...` 통합 matcher 분할).
...
1. `plugin.json` — 최소 필드 `{ "name": "transcodes-guard", "description": "..." }`. Antigravity 2.0는 `name` 생략시 디렉토리명을 자동 사용.
...
3. `hooks/hooks.json` — **Claude Code 매니페스트를 거의 그대로 복사 가능**:
   ```json
   {
     "description": "transcodes-guard step-up MFA gate",
     "hooks": {
       "PreToolUse": [
         {
...
- **테스트 시그널 부재** — 현 transcodes-guard는 hook smoke test가 1종(Claude Code)뿐. 호스트별 smoke test fixture 추가 필요 (각 도구의 stdin JSON 예시를 `tests/fixtures/{host}/`에 두기). Antigravity는 wire format 미확정이므로 fixture 자체가 잠정.
