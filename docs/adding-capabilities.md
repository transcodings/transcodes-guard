...
이 문서는 새 도구·리소스·프롬프트를 추가할 때 따라 하는 체크리스트다. 모든 작업은 **`packages/mcp-server-core/src/server.ts` 안의 `createServer()`** 한 함수에서 끝난다.
...
여러 도구가 쌓이면 `packages/mcp-server-core/src/server.ts`가 비대해진다. 그 시점에 분리 고려:

- 도구별 파일 분리: 개별 도구 구현을 별도 모듈로 빼고 `server.ts`는 등록만 담당.
- 일괄 등록 헬퍼로 묶기.

Transcodes 백엔드 결합 도구는 이미 `packages/transcodes-mcp-tools/`로 분리돼 `GateBackend.registerBackendTools()`로 주입된다.
