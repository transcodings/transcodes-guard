...
const MCP_TOOL_NAME_GUIDANCE = 'MCP tool name as emitted by the host PreToolUse hook (full wire name). Call simulate_tool_call with the same string to verify before saving.';
...
export function createServer(backend = getGateBackend()) {
    ...
    server.registerTool('update_tool_rule', {
        ...
        inputSchema: {
            ...
            resource: z.string().min(1).optional().describe(TOOL_RULE_RBAC_GUIDANCE),
            action: z
                .enum(['create', 'read', 'update', 'delete'])
                .optional()
                .describe('RBAC CRUD action this tool maps onto.'),
