...
export function createServer() {
    ...
    server.registerResource("danger-patterns", "danger-patterns://list", {
        ...
        description: "Regex patterns the PreToolUse hook uses to block dangerous Bash commands. Merges immutable system patterns (hooks/danger-patterns.json) with user patterns (~/.claude/ai-action-tracker/user-patterns.json), read fresh at every request.",
    ...
    server.registerTool("simulate_command", {
        ...
        description: "Check whether a Bash command would be blocked by the regex layer of the PreToolUse hook. Runs against the union of system and user patterns. Does NOT simulate the second-layer `rm -rf` git-tracked check (which is cwd-dependent) — the hook may still block commands this tool reports as allowed.",
    ...
    server.registerTool("add_user_pattern", {
        ...
        description: "Register a new user-owned block pattern. id must be unique across both system and user patterns. regex must compile. Persisted to ~/.claude/ai-action-tracker/user-patterns.json and effective on the next hook invocation.",
        inputSchema: {
            id: z
                .string()
    ...
    server.registerTool("update_user_pattern", {
        ...
        description: "Modify regex or reason of an existing user pattern. System patterns cannot be modified — attempts are rejected. Pass only the fields you want to change.",
        inputSchema: {
            id: z.string().min(1),
    ...
    server.registerTool("remove_user_pattern", {
        ...
        description: "Delete an existing user pattern by id. System patterns cannot be removed — attempts are rejected.",
