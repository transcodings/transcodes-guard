...
function legacyCacheDir() {
  if (process.platform === 'win32') {
    const base2 =
      ...
      path2.join(os2.homedir(), 'AppData', 'Local');
  ...
  const base =
    xdg && xdg.length > 0 ? xdg : path2.join(os2.homedir(), '.cache');
...
function loadStepupConfig() {
  ...
  if (!tokenRaw) {
    throw new Error(
      'No Transcodes token found. Get a token from the Transcodes console (member detail page, https://app.transcodes.io), then run `transcodes login <token>` in a terminal \u2014 or set the TRANSCODES_TOKEN environment variable.'
    );
  }
...
function readVerified() {
  ...
  if (ageMs > STEPUP_TTL_MS) {
    process.stderr
      .write(`transcodes-guard: verified record EXPIRED (sid=${sid}, age=${ageMs}ms, ttl=${STEPUP_TTL_MS}ms) \u2014 starting a new step-up.
`);
    consumeVerified();
    return null;
  }
...
var danger_patterns_default = {
  patterns: [
    ...
    },
    {
      ...
      regex: '\\btranscodes\\b[^\\n]*\\bdisable\\b',
      reason:
        'Disabling the transcodes-guard step-up gate \u2014 requires human step-up approval (an agent must not silently switch off its own guardrails)',
    },
  ],
};
...
function validateNewPattern(input) {
  ...
  if (detectMcpToolName(regex)) {
    throw new PatternValidationError(
      `"${regex}" looks like an MCP tool name, not a Bash command pattern. Bash Command only match Bash command strings via regex; they never match MCP tool calls. Use add_tool_rule (exact tool_name match) instead.`
    );
  }
  try {
    new RegExp(regex);
...
var tool_rules_default = {
  rules: [
    ...
    },
    {
      ...
      toolName:
        'mcp__plugin_transcodes-guard_transcodes-guard__unsuspend_member',
    ...
    },
    {
      ...
      toolName:
        'mcp__plugin_transcodes-guard_transcodes-guard__set_role_permissions',
    ...
    },
    {
      ...
      toolName:
        'mcp__plugin_transcodes-guard_transcodes-guard__update_member_role',
    ...
    },
    {
      ...
      toolName:
        'mcp__plugin_transcodes-guard_transcodes-guard__retire_resource',
    ...
    },
    {
      ...
      toolName:
        'mcp__plugin_transcodes-guard_transcodes-guard__passcode_create',
...
function validateNewToolRule(input) {
  ...
  if (detectShellCommand(trimmedToolName)) {
    throw new ToolRuleValidationError(
      `"${trimmedToolName}" looks like a Bash command, not an MCP tool name. Tool rules match a tool_name exactly (e.g. mcp__github__delete_repository); they never match Bash commands. Use add_user_pattern (regex) instead.`
    );
  }
  const trimmedReason = reason.trim();
  if (!trimmedReason) {
...
async function createStepupSession(config, args) {
  ...
  return {
    ...
    browserUrl: payload
      ? readString2(payload, 'url') ??
        readString2(payload, 'browser_url') ??
        readString2(payload, 'browserUrl')
      : void 0,
    expiresAt: payload
      ? readString2(payload, 'expiresAt') ?? readString2(payload, 'expires_at')
      : void 0,
  };
}
...
function checkRmGitTracked(command, cwd) {
  ...
  return {
    ...
    details: hits.map((h) => {
      ...
      return `${h.target} \u2014 ${
        h.trackedCount
      } tracked file(s): ${h.samples.join(', ')}${more}`;
    }),
    command,
  };
}
function stringifyToolInput(input) {
  try {
    ...
    if (s === void 0) return '[unserializable]';
  ...
  const gateInput =
    classified.kind === 'bash'
      ...
      : {
          ...
          fingerprintKey: `${classified.toolName}:${JSON.stringify(
            classified.toolInput
          )}`,
...
function formatStepupFailureReason(decision) {
  return `Bash blocked by transcodes-guard: ${
...
function inspectPending(now) {
  ...
  const expiresAt =
    typeof data.expiresAt === 'string' ? data.expiresAt : void 0;
