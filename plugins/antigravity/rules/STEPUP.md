...
When a `PreToolUse` hook denies a shell or MCP tool call with `decision: "deny"` and the reason mentions Step-up MFA, the command was **BLOCKED and did NOT execute**. Drive the loop deterministically — **do NOT wait for user confirmation between steps**:
...
3. On `outcome: "verified"` retry the **same** `run_command` or MCP call — the hook detects the verified state locally and allows it. On `outcome: "timeout"` ask the user to retry WebAuthn, then call the wait tool again.
...
To simulate a hook invocation end-to-end with state diff, call `simulate_hook_invocation`. It spawns the actual hook binary in a subprocess.
