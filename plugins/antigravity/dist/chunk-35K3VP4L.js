...
async function createStepupSession(config, args) {
  ...
  return {
    envelope,
    sid: payload ? readString(payload, "sid") : void 0,
    browserUrl: payload ? readString(payload, "url") ?? readString(payload, "browser_url") ?? readString(payload, "browserUrl") : void 0,
    expiresAt: payload ? readString(payload, "expiresAt") ?? readString(payload, "expires_at") : void 0,
    mode: payload ? readString(payload, "mode") : void 0
  };
}
...
async function pollStepupSession(config, sid) {
  ...
  return {
    envelope,
    status: payload ? readString(payload, "status") : void 0
  };
}
async function pollStepupByCoordinate(config, coordinate) {
  ...
  const payload = readStepupPayload(envelope);
  const sid = payload ? readString(payload, "sid") : void 0;
  return {
    envelope,
    status: payload ? readString(payload, "status") : void 0,
    ...sid ? { sid } : {}
...
function parseMemberAccessToken(rawToken) {
  ...
  const organizationId = readString2(payload, "oid");
  const projectId = readString2(payload, "pid");
  const memberId = readString2(payload, "mid");
  ...
  return {
    ...
    claims: {
      ...
      aud,
      exp,
      iss: readString2(payload, "iss"),
      jti: readString2(payload, "jti"),
