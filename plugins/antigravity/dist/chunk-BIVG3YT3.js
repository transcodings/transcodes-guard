...
function messageText(message) {
  ...
  const text = content.map((block) => {
    if (!block || typeof block !== "object")
      return void 0;
    const b = block;
    return b.type === "text" ? readString(b.text) : void 0;
...
function knownRecordContext(record) {
  ...
  if (record.source === "USER_EXPLICIT" && record.type === "USER_INPUT") {
    ...
    return { prompt: readString(request2) ?? content };
  }
  if (record.type !== "event_msg")
    return void 0;
  const payload = record.payload;
  return payload?.type === "user_message" ? { prompt: readString(payload.message) } : {};
}
function genericUserPrompt(record) {
  const role = readString(record.role) ?? readString(record.message?.role) ?? readString(record.type);
  if (role !== "user" && role !== "user_message")
    return void 0;
  if (isMetaRecord(record))
    return void 0;
  const text = messageText(record.message) ?? readString(record.content) ?? readString(record.text) ?? readString(record.message);
...
function summarizePromptWithTitle(transcriptPath, prompt) {
  ...
  const parts = [title, prompt].filter((part) => readString(part) !== void 0);
...
function unwrapMcpDispatch(name, rawArgs) {
  ...
  const a = rawArgs;
  const inner = readString2(a.ToolName) ?? readString2(a.toolName);
  if (!inner)
    return { toolName: name, toolArgs: rawArgs };
...
var antigravityAdapter = {
  ...
  parsePreToolUseStdin(raw) {
    ...
    const { toolName, toolArgs } = unwrapMcpDispatch(readString2(toolCall?.name), toolCall?.args);
    ...
    return {
      ...
      sessionId: readString2(payload.conversationId),
      ...
      agentModel: readString2(payload.modelName),
      transcriptPath: readString2(payload.transcriptPath),
  ...
  parsePreInvocationStdin(raw) {
    ...
    return {
      ...
      conversationId: readString2(payload.conversationId),
      transcriptPath: readString2(payload.transcriptPath),
      workspacePaths: readStringArray(payload.workspacePaths),
      artifactDirectoryPath: readString2(payload.artifactDirectoryPath)
    };
  },
  ...
  emitSessionStartContext(_additionalContext) {
    throw new Error("Antigravity has no SessionStart hook event. Use PreInvocation with invocationNum=0.");
  },
