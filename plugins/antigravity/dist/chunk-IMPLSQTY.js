...
function summarizeTasks(transcriptPath) {
  ...
  for (const record of tailJsonlLines(transcriptPath)) {
    ...
    const role = readString(r.role) ?? readString(r.message?.role) ?? readString(r.type);
    if (role !== "user" && role !== "user_message")
      continue;
    if (isMetaRecord(r))
      continue;
    const text = messageText(r.message) ?? readString(r.content) ?? readString(r.text) ?? readString(r.message);
