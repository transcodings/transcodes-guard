...
function hasLatch(sid, resource, action) {
  try {
    readFileSync2(latchPath(sid, resource, action), "utf8");
    return true;
  } catch {
    return false;
  }
}
function writeLatch(sid, resource, action, authSid, now = Date.now()) {
  const record = {
    sid,
    resource,
    action,
