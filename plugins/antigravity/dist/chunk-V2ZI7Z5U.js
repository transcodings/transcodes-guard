...
var GUARD_PROTECTED_TOOL_RULES = [
  {
    id: "tc-retire-member",
...
var TRANSCODES_GUARD_TOOL_PREFIX = "tc_";
...
// ../../packages/core/dist/stepup/verified-memory.js
var verified = /* @__PURE__ */ new Map();
function markStepupVerified(sid, now = Date.now()) {
  if (sid)
    verified.set(sid, now);
}
function claimStepupVerified(now = Date.now()) {
...
var SYSTEM_PROTECTED_TOOL_RULES = GUARD_PROTECTED_TOOL_RULES.map((r) => ({
  ...r,
  type: "mcp",
  matcher: "exact",
  source: "system"
}));
function resolveProtectedToolRule(toolName, rules = SYSTEM_PROTECTED_TOOL_RULES) {
  return rules.find((r) => {
    if (!ruleAppliesToHost(r))
      return false;
    if (r.name === toolName)
      return true;
    if (toolName.startsWith("mcp__") || toolName.includes(TRANSCODES_GUARD_TOOL_PREFIX)) {
...
async function execProtectedTool(toolName, run) {
  ...
  if (rule?.action !== void 0 && rule.resource !== void 0) {
    ...
    if (level === 1) {
      return {
        isError: false,
        content: [{ type: "text", text: await run(void 0) }]
      };
    }
    const sid2 = claimStepupVerified();
    if (!sid2)
      return stepupRequiredResult(toolName, rule);
    return {
      isError: false,
      content: [{ type: "text", text: await run(sid2) }]
    };
  }
