...
토큰이 없으면 hook은 여전히 위험 명령을 **차단**하지만 스텝업 세션을 시작할 수 없습니다 — Antigravity가 토큰을 제공하라는 사유를 표시합니다. 영구적이고 권장되는 방법은 CLI 컨트롤 플레인입니다: `transcodes login <token>`(`@bigstrider/transcodes-cli`)이 토큰을 `~/.transcodes/config.json`에 기록하면, hook이 환경 변수 없이 그 파일에서 읽습니다.
