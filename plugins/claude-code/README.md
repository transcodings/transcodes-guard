...
차단 시 `transcodes-guard: BLOCKED (사유) — <명령>` 메시지가 stderr로 출력되고 명령이 실행되지 않음.

### MCP 서버 (`transcodes-guard`)
...
> `echo` tool과 `greeting` prompt는 아직 placeholder입니다. 향후 secrets 검사·MCP 서버 위험 프로파일 등 보안 advisory tool이 추가될 예정 — 리포지토리 [`docs/prd/`](https://github.com/transcodings/transcodes-guard/tree/main/docs/prd) 참고.
