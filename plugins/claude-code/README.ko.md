...
- `TRANSCODES_TOKEN`으로 내보낸 **멤버 MCP JWT** ([환경 변수](#환경-변수) 참고).
...
토큰이 없으면 hook은 여전히 위험 명령을 **차단**하지만 스텝업 세션을 시작할 수 없습니다 — 차단 사유에 `TRANSCODES_TOKEN`을 설정하라는 안내가 표시됩니다. 사람용 컨트롤 플레인(`@bigstrider/transcodes-cli`, `transcodes` 명령)이 토큰을 `~/.transcodes/config.json`에 기록하며, 이를 사용하면 hook이 그 파일에서 읽으므로 환경 변수는 선택 사항이 됩니다.
...
| 변수 | 필수 여부 | 용도 |
|---|---|---|
| `TRANSCODES_TOKEN` | 예 (스텝업 작동에 필요) | `x-transcodes-token`으로 전송되는 멤버 MCP JWT. |
...
로컬 스텝업 상태는 `~/.transcodes/state/` 아래에 있으며, 설계상 **모든 transcodes-guard 플러그인이 공유**합니다 — 모든 호스트가 동일한 Transcodes 백엔드와 통신하므로 한 호스트에서 verified 된 세션이 다른 호스트로 이어집니다. 동시 사용이 지원되며, verified 레코드에 대한 같은-초 경쟁은 알려진 한계로, 백엔드의 sid-replay 보호가 권위 있는 백스톱입니다.
