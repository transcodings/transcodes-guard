...
둘 다 없으면 hook은 여전히 위험 명령을 **차단**하지만 스텝업 세션을 시작할 수 없습니다 — 차단 사유에 토큰을 제공하라는 안내가 표시됩니다.
...
| 변수 | 필수 여부 | 용도 |
|---|---|---|
| `TRANSCODES_TOKEN` | config-less 환경(CI) fallback (저장 토큰 없을 때만) | `x-transcodes-token`으로 전송되는 멤버 MCP JWT. CLI 저장 시 생략 가능. |
