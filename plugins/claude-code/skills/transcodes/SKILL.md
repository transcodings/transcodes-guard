...
4) Create, review, extract, edit, deploy, or share a Persona onboarding kit
   ...
   - EXTRACT — trigger when the user asks to extract, migrate, or import an existing project's agent settings into a new Persona. Extract migrates project settings into a new Persona; it is not CREATE or Diet, does not use CREATE's no-preview rule, and must never invent or add a `transcodes persona extract` CLI command.
     ...
     4. For each discovered Skill or explicit relative `@file`/companion reference, inspect referenced files statically without running them. Include only safe, relevant companions that stay inside the source Skill and preserve their Skill-relative paths. Report unreferenced companions as excluded. Never execute scripts, including scripts that appear safe or are referenced by the discovered instructions.
     ...
     6. Analyze duplicates, conflicts, host-only metadata, and name collisions before asking for a Persona name. Merge byte-identical or meaning-identical duplicates once and record every source; never silently choose between conflicting directives. If no extractable Instruction, Rule, or Skill remains, or any required classification is unsafe or conflict is unresolved, explain why and create nothing.
