...
4) Create, review, edit, deploy, or share a Persona onboarding kit
   ...
   - KNOWLEDGE BASE — `knowledge-base` is a reserved system Skill created with every Persona, including every template. It ships with a starter document that explains what belongs there. Never save, rewrite, or delete its SKILL.md and never add scripts, assets, or folders inside it. Put durable facts the agent must not guess — product names, tokens, approved claims, contracts — each as `references/<kebab-case>.md` (lowercase kebab-case only, e.g. `design-tokens.md`) with a Title and a Description that answers when that entry should be referenced. Apply copies these files to `.agents/references` and writes their names, descriptions, and deployed paths into the generated Instruction. The deployed agent reads every file whose description matches information the request needs; when matched facts overlap, a narrower product, campaign, or task scope overrides a broader scope and non-conflicting facts merge. If no file matches, say the information is not in the Knowledge Base, then label any general knowledge or ask for the missing fact; never invent a project-specific fact. Keep Knowledge Base facts distinct from search and general suggestions. The final Transcodes Knowledge field lists the exact names of every file used, or `none` when no file matched. Create or update knowledge only with `transcodes persona save --persona <name> --kind skill --name knowledge-base --file references/<kebab-case>.md --content-file <path>`; each Markdown file must have non-empty `name` and `description` frontmatter. Delete one with `transcodes persona delete-reference --persona <name> --file references/<kebab-case>.md`. The CLI creates a missing knowledge-base automatically and regenerates only the `# References` index from every reference file. Do not put Must/Never policy or step-by-step workflows here.
   ...
   - Before applying, show one concise confirmation containing the Persona name, exact absolute destination (project path or home), selected apps, and that existing generated agent files for those targets may be replaced. For a project folder, also state that generated files there which this Persona no longer produces will be deleted. Global apply replaces the current Persona knowledge files under the device-wide agent directory and leaves files outside Persona-generated outputs alone. Deployment is a mutating action: do not run it until the user confirms. Never pass `--yes` until that confirmation is explicit.
