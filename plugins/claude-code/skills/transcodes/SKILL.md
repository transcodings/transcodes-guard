...
4) Create, review, extract, edit, deploy, or share a Persona onboarding kit
   ...
   - EXTRACT — trigger when the user asks to extract, migrate, or import an existing project's agent settings into a new Persona. Extract migrates project settings into a new Persona; it is not CREATE or Diet, does not use CREATE's no-preview rule, and must never invent or add a `transcodes persona extract` CLI command.
     1. Require the source project path. If it is missing, ask for it. Resolve it to an absolute path and confirm it exists, is a readable directory, and can be listed; otherwise report the invalid or unreadable path and create nothing.
     ...
     7. Ask for a new lowercase-kebab-case Persona name, then run `transcodes persona list` and refuse any name that already exists. Preserve each valid lowercase-kebab-case Rule and Skill name. Propose renames only for invalid names, the reserved `knowledge-base` name, or collisions with different content, and require the user to confirm every rename or conflict resolution.
