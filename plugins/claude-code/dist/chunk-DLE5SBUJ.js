...
function formatStepupChallengedSystemMessage(decision) {
  ...
  return appendBackendReasoning([
    ...
    '  3. On `outcome: "verified"` retry the SAME Bash command \u2014 the hook detects the verified state and allows it. On `outcome: "timeout"` ask the user to complete WebAuthn and whether to retry; only call the wait tool again if they say yes. On `outcome: "rejected"` or `outcome: "not_found"` stop \u2014 the user declined or abandoned step-up; do NOT retry the command unless they explicitly ask.'
  ].join("\n"), decision.reasoning);
}
function formatStepupRejectedReason(decision) {
  return `Step-up MFA was declined. Tell the user this command did not run. Do not retry or poll unless they explicitly ask to authenticate again (${decision.resource}/${decision.action}).`;
}
...
function formatStepupRejectedSystemMessage(decision) {
  return appendBackendReasoning([
    ...
    "  2. Do NOT call `tc_poll_stepup_session_wait` or `tc_poll_stepup_session` \u2014 the challenge is terminal.",
    "  3. Do NOT retry this command or run alternate Bash/MCP tools to work around the block in this turn.",
    "  4. Wait for the user. Only start step-up again if they explicitly ask to authenticate and retry.",
    "",
    "Protocol: rejected is terminal \u2014 do not retry commands or poll tools until the user explicitly asks.",
