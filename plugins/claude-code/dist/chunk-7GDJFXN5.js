...
// ../../packages/core/dist/patterns/data/tool-rules.json
...
// ../../packages/core/dist/stepup/latch.js
import { readdirSync, readFileSync as readFileSync2, rmSync as rmSync2, writeFileSync as writeFileSync2 } from "fs";
import path3 from "path";
var LATCH_PREFIX = "step-up.";
var LATCH_SUFFIX = ".json";
function listLatches(now = Date.now()) {
  let names;
  try {
  ...
  for (const name of names) {
    ...
    try {
      const parsed = JSON.parse(readFileSync2(path3.join(cacheDir(), name), "utf8"));
      const group = parsed?.group?.trim();
      if (!parsed || !group || typeof parsed.resource !== "string" || typeof parsed.action !== "string" || typeof parsed.createdAt !== "number") {
        continue;
      }
      out.push({
        group,
...
// ../../packages/core/dist/stepup/sid.js
import { randomBytes } from "crypto";
import { readFileSync as readFileSync3, writeFileSync as writeFileSync3 } from "fs";
import path4 from "path";
var GROUP_FILE = "grouping-session.json";
function groupPath() {
  return path4.join(cacheDir(), GROUP_FILE);
}
function readRecord(now) {
  let raw;
  try {
    raw = readFileSync3(groupPath(), "utf8");
  } catch {
    return null;
  }
  try {
...
function peekPromptGroup(now = Date.now()) {
...
function registerJwkTools(server) {
  server.registerTool("tc_jwk_backup", {
