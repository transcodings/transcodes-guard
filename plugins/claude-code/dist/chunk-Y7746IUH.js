...
// ../../packages/stepup-core/dist/latch.js
...
function latchPath(sid, resource, action) {
  return path3.join(cacheDir(), latchName(sid, resource, action));
}
function parseLatchRecord(file, now) {
  ...
  try {
    ...
    if (!parsed || typeof parsed.sid !== "string" || typeof parsed.resource !== "string" || typeof parsed.action !== "string" || typeof parsed.createdAt !== "number") {
    ...
    return {
      ...
      authSid: parsed.authSid?.trim() || void 0,
...
function readLatchRecord(sid, resource, action, now = Date.now()) {
  return parseLatchRecord(latchPath(sid, resource, action), now);
}
function hasLatch(sid, resource, action, now = Date.now()) {
  return readLatchRecord(sid, resource, action, now) !== null;
}
function writeLatch(sid, resource, action, authSid, now = Date.now(), remindedCount) {
  const record = {
    sid,
    resource,
    action,
    authSid: authSid?.trim() || void 0,
    createdAt: now,
    remindedCount: typeof remindedCount === "number" && Number.isInteger(remindedCount) && remindedCount >= 0 ? remindedCount : void 0
  };
  try {
    writeFileSync2(latchPath(sid, resource, action), JSON.stringify(record), {
      mode: 384
    });
  } catch {
  }
}
function clearLatch(sid, resource, action) {
  try {
    rmSync2(latchPath(sid, resource, action), { force: true });
  } catch {
  }
}
function clearLatchByAuthSid(authSid) {
...
function incrementLatchRemindedCount(sid, resource, action, now = Date.now()) {
...
function formatStopReminderMessage(latch) {
  const authSid = latch.authSid?.trim() || "(use the tc_stepup_ sid from the PreToolUse deny message)";
  return [
...
// ../../packages/stepup-core/dist/sid.js
import { randomBytes } from "crypto";
import { readFileSync as readFileSync3, writeFileSync as writeFileSync3 } from "fs";
import path4 from "path";
var SID_FILE = "grouping-session.json";
function sidPath() {
  return path4.join(cacheDir(), SID_FILE);
}
function mintSid() {
...
function persist(record) {
  try {
    writeFileSync3(sidPath(), JSON.stringify(record), { mode: 384 });
  } catch {
  }
}
function readRecord(now) {
  let raw;
  try {
    raw = readFileSync3(sidPath(), "utf8");
  } catch {
    return null;
  }
  try {
...
function rotatePromptSid(now = Date.now()) {
  const record = { sid: mintSid(), createdAt: now };
  persist(record);
  return record.sid;
}
function resolvePromptSid(now = Date.now()) {
  const existing = readRecord(now);
  if (existing)
    return existing.sid;
  return rotatePromptSid(now);
}
function peekPromptSid(now = Date.now()) {
...
// ../../packages/stepup-core/dist/build-info.js
var PLUGIN_VERSION = "0.41.2";
...
function inspectStepupState(now = Date.now()) {
  return {
    ...
    prompt_sid: peekPromptSid(now),
    latches: listLatches(now)
  };
}

// ../../packages/stepup-core/dist/verified-memory.js
...
function formatStderrTag(decision) {
  switch (decision.kind) {
    case GATE_DECISION_KIND2.PROCEED_UNGATED:
    case GATE_DECISION_KIND2.PROCEED_BY_POLICY:
      return "transcodes-guard: pass";
    case GATE_DECISION_KIND2.BLOCK_NO_TOKEN:
      return `transcodes-guard: BLOCKED (no token) \u2014 ${decision.block.command}`;
    case GATE_DECISION_KIND2.BLOCK_BY_POLICY:
    ...
    case GATE_DECISION_KIND2.BLOCK_STEPUP_CHALLENGED:
      return `transcodes-guard: STEPUP-PENDING sid=${decision.sid} \u2014 ${decision.block.command}`;
    case GATE_DECISION_KIND2.BLOCK_STEPUP_REJECTED:
...
export {
  ...
  objectType,
  external_exports,
  GATE_DECISION_KIND2 as GATE_DECISION_KIND,
