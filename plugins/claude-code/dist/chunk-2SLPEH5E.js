...
function latchPath(group, resource, action) {
  return path3.join(cacheDir(), latchName(group, resource, action));
}
function parseLatchRecord(file, now) {
  ...
  try {
    ...
    return {
      ...
      remindedCount: typeof parsed.remindedCount === "number" && Number.isInteger(parsed.remindedCount) && parsed.remindedCount >= 0 ? parsed.remindedCount : void 0
    };
  } catch {
    try {
      rmSync2(file, { force: true });
...
function readLatchRecord(group, resource, action, now = Date.now()) {
  return parseLatchRecord(latchPath(group, resource, action), now);
}
function hasLatch(group, resource, action, now = Date.now()) {
  return readLatchRecord(group, resource, action, now) !== null;
}
function writeLatch(group, resource, action, sid, now = Date.now(), remindedCount) {
  ...
  const record = {
    ...
    sid: trimmed,
    createdAt: now,
    remindedCount: typeof remindedCount === "number" && Number.isInteger(remindedCount) && remindedCount >= 0 ? remindedCount : void 0
  };
  try {
    writeFileSync2(latchPath(group, resource, action), JSON.stringify(record), {
      mode: 384
    });
  } catch {
  }
}
function clearLatch(group, resource, action) {
  try {
    rmSync2(latchPath(group, resource, action), { force: true });
  } catch {
  }
}
function clearLatchBySid(sessionSid) {
  ...
  for (const name of names) {
    ...
    const file = path3.join(cacheDir(), name);
    try {
...
function readSinglePendingLatchSid(group, now = Date.now()) {
  ...
  for (const name of names) {
    ...
    const parsed = parseLatchRecord(path3.join(cacheDir(), name), now);
...
function incrementLatchRemindedCount(group, resource, action, now = Date.now()) {
...
function formatStopReminderMessage(latch) {
...
function sweepLatches(now = Date.now()) {
  let names;
  try {
    names = readdirSync(cacheDir());
  } catch {
...
function persist(record) {
  try {
    writeFileSync3(groupPath(), JSON.stringify(record), { mode: 384 });
  } catch {
  }
}
...
function rotatePromptGroup(now = Date.now()) {
  const record = { group: mintGroup(), createdAt: now };
  persist(record);
  return record.group;
}
function resolvePromptGroup(now = Date.now()) {
...
var TRANSCODES_MCP_HOST_MARKER = "mcp_plugin_transcodes_guard";
...
function isTranscodesGuardWireToolName(toolName) {
  ...
  return lower.includes(TRANSCODES_GUARD_TOOL_PREFIX) || lower.startsWith(TRANSCODES_MCP_HOST_MARKER) || lower.includes("transcodes") || lower.includes("version");
}
...
var PLUGIN_VERSION = "0.49.2";

// ../../packages/core/dist/stepup/decision-audit.js
    ...
    const env = await request(config, {
      ...
      body: {
        ...
        metadata: {
          ...
          ...process.env.TRANSCODES_GUARD_HOST ? { host: process.env.TRANSCODES_GUARD_HOST } : {},
...
function formatStepupChallengedReason(decision) {
  return `Step-up MFA pending. sid=${decision.sid}. Open ${decision.browserUrl}, complete WebAuthn, then call MCP tool \`tc_poll_stepup_session_wait\` with sid="${decision.sid}" and retry the same Bash command.`;
}
function formatStepupChallengedSystemMessage(decision) {
  ...
  return appendBackendReasoning([
    ...
    `  2. Immediately call the MCP tool \`tc_poll_stepup_session_wait\` with sid="${decision.sid}". It blocks until verified or 60s timeout \u2014 one call replaces the polling loop.`,
