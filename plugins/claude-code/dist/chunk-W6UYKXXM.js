...
var CLAUDE_INTERNAL_TOOL_REGEX = /^(ToolSearch|TodoWrite|TodoRead|TaskCreate|TaskUpdate|TaskList|AskUserQuestion|EnterPlanMode|ExitPlanMode|exit_plan_mode)$/;
var CLAUDE_GATED_WIRE_NAMES = /* @__PURE__ */ new Set(["Bash"]);
...
var CURSOR_INTERNAL_TOOL_REGEX = /^(Read|Write|Grep|Delete|Glob|SemanticSearch|StrReplace|SwitchMode|TodoWrite|todo_write|AskQuestion|ask_question|Task|WebSearch|WebFetch|GenerateImage|EditNotebook|Await)$/i;
var CURSOR_GATED_WIRE_NAMES = /* @__PURE__ */ new Set(["Shell"]);
...
var ANTIGRAVITY_INTERNAL_TOOL_REGEX = /^(?!mcp__)(editor|files?|filesystem|code[_-]?search|file[_-]?editing|web[_-]?search|browser|browser[_-]?subagent|artifacts?|task[_-]?plan|implementation[_-]?plan|walkthrough|subagents?)([._:-]?[A-Za-z0-9_]+)*$/i;
var ANTIGRAVITY_GATED_WIRE_NAMES = /* @__PURE__ */ new Set(["run_command", "call_mcp_tool"]);
...
function isHostMetaTool(name) {
  return isClaudeInternalTool(name) || isCursorInternalTool(name) || isCodexInternalTool(name) || isAntigravityInternalTool(name);
}
...
function formatStepupPendingSystemMessage(decision) {
  ...
  return appendBackendReasoning([
    ...
    '  3. On `outcome: "verified"` retry the SAME Bash command \u2014 the hook detects the verified state and allows it. On `outcome: "timeout"` ask the user to retry WebAuthn, then call the wait tool again. On `outcome: "rejected"` or `outcome: "not_found"` stop \u2014 the user declined or abandoned step-up; do NOT retry the command unless they explicitly ask.'
  ].join("\n"), decision.reasoning);
}
function formatStepupRejectedReason(decision) {
  return `Step-up MFA was declined for this action. Do not poll or retry the command (${decision.resource}/${decision.action}) unless the user explicitly asks.`;
}
function formatStepupRejectedSystemMessage(decision) {
  return appendBackendReasoning([
    "\u{1F510} BLOCKED \u2014 Step-up MFA declined. This command was NOT executed.",
    "",
    `Reason : ${decision.block.reason}`,
    `Command: ${decision.block.command}`,
    "",
    "The user rejected WebAuthn for this grouped challenge. Do NOT call `poll_stepup_session_wait` or retry this command unless the user explicitly asks to authenticate again."
  ].join("\n"), decision.reasoning);
}
