...
리포지토리는 `.agents/plugins/marketplace.json`에 Codex 마켓플레이스 카탈로그(`./plugins/codex`를 가리킴)를 제공하며, `dist/`가 커밋되어 있습니다. `codex plugin marketplace add`는 GitHub 리포지토리를 직접 받으므로(Codex가 대신 클론), **수동 클론이나 빌드가 필요 없습니다**:
...
재현 가능한 설치를 원하면 GitHub에서 릴리스 태그를 골라 `--ref`로 전달하세요. 위의 핀 없는 명령은 현재 마켓플레이스 소스를 따릅니다.

> **저자 / 로컬 개발.** 원격 리포지토리 대신 로컬 체크아웃에서 설치하려면, 클론한 뒤 커밋된 `dist/`를 빌드하고 작업 사본을 `local` 마켓플레이스로 등록하세요:
>
> ```bash
> git clone https://github.com/transcodings/transcodes-guard.git
> cd transcodes-guard && npm install && npm run build:plugin
> codex plugin marketplace add .                 # 로컬 마켓플레이스 등록
...
토큰이 없으면 hook은 여전히 위험 명령을 **차단**하지만 스텝업 세션을 시작할 수 없습니다 — Codex가 토큰을 제공하라는 사유를 표시합니다.
...
토큰 해석: 토큰은 `~/.transcodes/config.json`(`transcodes` 대시보드 또는 `transcodes set`)에서만 읽습니다.
