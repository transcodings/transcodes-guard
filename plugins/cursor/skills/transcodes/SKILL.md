...
4) Create, review, extract, edit, deploy, or share a Persona onboarding kit
   ...
   - EXTRACT — trigger when the user asks to extract, migrate, or import an existing project's agent settings into a new Persona. Extract migrates project settings into a new Persona; it is not CREATE or Diet, does not use CREATE's no-preview rule, and must never invent or add a `transcodes persona extract` CLI command.
     ...
     2. Keep discovery read-only and inside that directory. Never modify source files, follow a symbolic link outside the source directory, infer instructions from general source code, or execute any discovered script. Inspect only these conventional project locations:
     ...
     3. Inventory each discovered path once even when several hosts share it. Classify it as Instruction, Rule, Skill, or excluded and map source path → target kind and name. One root/common Instruction becomes the Persona Instruction; map additional scoped instructions to Rules only when their scope can be preserved safely, otherwise mark them conflicted or unsafe. Preserve wording and valid metadata; translate only host-specific activation metadata needed by the target, and report every unsupported host-only field.
     ...
     8. Before writing, show one complete preview with the new Persona name; the built-in `knowledge-base` Skill that create adds; every Instruction, Rule, Skill, and included companion; and a source path → target mapping labeled preserved, merged, renamed, conflicted, or excluded with the reason. Include every merge, exclusion, host-metadata, and conflict decision, then require explicit approval of that exact preview.
     ...
     10. If create or any save fails, stop immediately, report the failing command and any partially created new Persona, and do not report success or continue to deploy. After all saves succeed, run `transcodes persona list --persona <name>` and `transcodes persona read` for every saved Instruction, Rule, Skill, and companion; compare them with the approved preview while treating the CLI's documented Instruction frontmatter/attribution and Skill name/companion-index normalization as matching, and report success only when all entries exist and otherwise match.
     11. Remove temporary files with the host native file tool. Do not deploy automatically; hand the verified new Persona to APPLY OR DEPLOY below, which requires its own confirmation.
