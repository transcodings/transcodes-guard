...
var cursorAdapter = {
  ...
  emitUserPromptSubmitContext(_additionalContext) {
    throw new Error("Cursor's beforeSubmitPrompt has no additional_context channel. The hook is inert \u2014 emit `{ continue: true }` directly.");
  },
  emitStop(reason) {
    if (!reason)
      return "{}";
...
function summarizeTasks(transcriptPath) {
  ...
  for (const record of tailJsonlLines(transcriptPath)) {
    ...
    const r = record;
    if (r.type === "ai-title") {
      title = readString2(r.aiTitle) ?? title;
      continue;
    }
    if (r.type === "last-prompt") {
      prompt = readString2(r.lastPrompt) ?? prompt;
      continue;
    }
    if (r.type === "event_msg") {
      const payload = r.payload;
      if (payload?.type === "user_message") {
        prompt = readString2(payload.message) ?? prompt;
      }
      continue;
    }
    const role = readString2(r.role) ?? readString2(r.message?.role) ?? readString2(r.type);
    if (role !== "user" && role !== "user_message")
      continue;
    if (isMetaRecord(r))
      continue;
    const text = messageText(r.message) ?? readString2(r.content) ?? readString2(r.text) ?? readString2(r.message);
    if (text && !COMMAND_ENVELOPE.test(text))
      prompt = text;
  }
  const parts = [title, prompt].filter((part) => part !== void 0);
...
async function reqEnvelope(config, input, toolName, pathSuffix) {
  ...
  const path3 = pathSuffix ? `${base}${pathSuffix}` : base;
  return request(config, { ...input, path: path3 });
}
async function req(config, input, toolName, pathSuffix) {
  ...
  const path3 = pathSuffix ? `${base}${pathSuffix}` : base;
  const envelope = await request(config, { ...input, path: path3 });
