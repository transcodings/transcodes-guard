...
function formatNoTokenReason(block) {
  return `Bash blocked by transcodes-guard: ${block.reason}. Step-up MFA gate is not configured (no Transcodes token found). Tell the user to ${cliInstallBlurb()} Run \`transcodes login\`, sign in with Google, and choose an organization. Manual fallback: \`transcodes set <token> -l <label>\`. Or run the command outside the agent.`;
}
...
function formatStepupCreateFailedDetail(decision) {
  ...
  return failure.reason === "no-token" ? `No Transcodes token found \u2014 step-up MFA gate is unavailable. ${cliInstallBlurb()} Run \`transcodes login\`, sign in with Google, and choose an organization. Manual fallback: \`transcodes set <token> -l <label>\`.` : failure.reason === "create-failed" ? `Step-up MFA session could not be started${failure.detail ? ` (${failure.detail})` : ""}.` : `Step-up MFA gate errored${failure.detail ? ` (${failure.detail})` : ""}.`;
}
