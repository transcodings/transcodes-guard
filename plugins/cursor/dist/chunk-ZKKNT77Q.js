...
// ../../packages/core/dist/hosts/claude-code.js
...
function parsePreToolUsePayload(raw) {
  try {
    ...
    return {
      ...
      promptId: readString(payload.prompt_id) ?? readString(payload.turn_id) ?? readString(payload.generation_id),
...
// ../../packages/core/dist/hosts/cursor.js
var cursorAdapter = {
  host: "cursor",
  ...
  emitUserPromptSubmitContext(_additionalContext) {
    throw new Error("Cursor's beforeSubmitPrompt has no additional_context channel. Capture locally, then emit `{ continue: true }` directly.");
  },
  emitStop(reason) {
    if (!reason)
      return "{}";
...
// ../../packages/core/dist/hosts/prompt-cache.js
import { createHash, randomBytes } from "crypto";
import { chmodSync, closeSync as closeSync2, existsSync as existsSync2, fsyncSync, lstatSync, mkdirSync as mkdirSync2, openSync as openSync2, readdirSync, readFileSync, renameSync as renameSync2, unlinkSync, writeFileSync } from "fs";
...
var HOST_ENV_VAR = "TRANSCODES_GUARD_HOST";
var PLUGIN_DATA_ENV = "PLUGIN_DATA";
...
// ../../packages/core/dist/hosts/transcript.js
...
function clip(text) {
  ...
  return flat.length <= MAX_PART_CHARS ? flat : `${flat.slice(0, MAX_PART_CHARS - 1)}\u2026`;
}
function messageText(message) {
  ...
  const text = content.map((block) => {
    if (!block || typeof block !== "object")
      return void 0;
    const b = block;
    return b.type === "text" ? readString2(b.text) : void 0;
  }).filter((part) => part !== void 0).join("\n");
  return readString2(text);
}
var COMMAND_ENVELOPE = /^<(command-(name|message|args)|local-command-std(out|err))>/;
function isMetaRecord(r) {
  return r.isMeta === true || r.message?.isMeta === true;
}
function knownRecordContext(record) {
  if (record.type === "ai-title") {
    return { title: readString2(record.aiTitle) };
  }
  if (record.type === "last-prompt") {
    return { prompt: readString2(record.lastPrompt) };
  }
  if (record.source === "USER_EXPLICIT" && record.type === "USER_INPUT") {
    const content = readString2(record.content);
    if (!content)
      return {};
    const request2 = /<USER_REQUEST>\s*([\s\S]*?)\s*<\/USER_REQUEST>/.exec(content)?.[1];
    return { prompt: readString2(request2) ?? content };
  }
  if (record.type !== "event_msg")
    return void 0;
  const payload = record.payload;
  return payload?.type === "user_message" ? { prompt: readString2(payload.message) } : {};
}
function genericUserPrompt(record) {
  const role = readString2(record.role) ?? readString2(record.message?.role) ?? readString2(record.type);
  if (role !== "user" && role !== "user_message")
    return void 0;
  if (isMetaRecord(record))
    return void 0;
  const text = messageText(record.message) ?? readString2(record.content) ?? readString2(record.text) ?? readString2(record.message);
  return text && !COMMAND_ENVELOPE.test(text) ? text : void 0;
}
...
function summarizePromptWithTitle(transcriptPath, prompt) {
  const title = readTranscriptContext(transcriptPath)?.title;
  const parts = [title, prompt].filter((part) => readString2(part) !== void 0);
  return parts.length ? parts.map(clip).join(" \xB7 ") : void 0;
}
function summarizeTasks(transcriptPath) {
  const context = readTranscriptContext(transcriptPath);
  if (!context)
    return void 0;
  const parts = [context.title, context.prompt].filter((part) => part !== void 0);
  return parts.length ? parts.map(clip).join(" \xB7 ") : void 0;
}
...
var MAX_CACHE_FILE_BYTES = MAX_TURNS * MAX_PROMPT_BYTES * 6 + 8 * 1024;
var TTL_MS = 24 * 60 * 60 * 1e3;
var DIAGNOSTICS_ENV = "TRANSCODES_GUARD_PROMPT_DIAGNOSTICS";
function cachePromptId(promptId) {
  const normalized = promptId?.trim();
  return normalized ? createHash("sha256").update(normalized).digest("hex") : void 0;
}
function currentHost(host) {
  return host ?? detectHost() ?? "unknown";
}
function cacheFileName(host, sessionId) {
  const digest = createHash("sha256").update(host).update("\0").update(sessionId).digest("hex");
  return `${digest}.json`;
}
...
function readCache(filePath) {
  ...
  if (stat.isSymbolicLink() || !stat.isFile() || stat.size > MAX_CACHE_FILE_BYTES) {
  ...
  const entries = parsed.entries.filter((entry) => {
    ...
    return typeof candidate.prompt === "string" && typeof candidate.capturedAt === "number" && Number.isFinite(candidate.capturedAt) && (candidate.promptId === void 0 || typeof candidate.promptId === "string");
  });
  return { version: CACHE_VERSION, entries };
}
function writeCache(filePath, cache) {
  const root = path2.dirname(filePath);
  const tempPath = path2.join(root, `.${path2.basename(filePath)}.${process.pid}.${randomBytes(6).toString("hex")}.tmp`);
  let fd;
  try {
...
function appendEntry(entries, entry, forceRefresh) {
  const next = entry.promptId ? entries.filter((candidate) => candidate.promptId !== entry.promptId) : entries;
  const isDuplicate = !entry.promptId && next.at(-1)?.prompt === entry.prompt && !forceRefresh;
  return isDuplicate ? void 0 : [...next, entry].slice(-MAX_TURNS);
}
function capturePromptUnsafe(input) {
  ...
  if (!root)
    return;
  const filePath = path2.join(root, cacheFileName(host, sessionId));
...
function resolvePromptContext(input) {
  const host = currentHost(input.host);
  const now = input.now ?? Date.now();
  const tasks = readCachedTasks(input, host, now);
  if (tasks) {
    const result2 = { tasks, source: "prompt_hook" };
    emitDiagnostic(host, result2.source);
    return result2;
  }
  const transcriptTasks = summarizeTasks(input.transcriptPath);
  const result = transcriptTasks ? { tasks: transcriptTasks, source: "transcript" } : { source: "absent" };
...
function readCachedTasks(input, host, now) {
  try {
    ...
    prune(root, now);
    return entry ? summarizePromptWithTitle(input.transcriptPath, entry.prompt) : void 0;
  } catch {
    return void 0;
  }
}

// ../../packages/core/dist/patterns/rbac.js
...
// ../../packages/core/dist/patterns/data/builtin-exempt/antigravity.json
var antigravity_default = [
  {
    name: "view_file",
    reason: "grade 2 (workspace read) \u2014 file read (antigravity.google/docs/hooks, 2026-07 doc-verified; live capture pending t4)"
  },
  {
    name: "list_dir",
    reason: "grade 2 (workspace read) \u2014 directory listing (doc-verified 2026-07; live capture pending t4)"
  },
  {
    name: "grep_search",
    reason: "grade 2 (workspace read) \u2014 text search (doc-verified 2026-07; live capture pending t4)"
  },
  {
    name: "ask_question",
    reason: "grade 1 (conversation) \u2014 prompts the user, no side effects (doc-verified 2026-07)"
  },
  {
    name: "ask_permission",
    reason: "grade 1 (conversation) \u2014 user permission prompt, no side effects (doc-verified 2026-07)"
  },
  {
    name: "list_permissions",
    reason: "grade 1 (conversation) \u2014 reads permission state (doc-verified 2026-07)"
  }
];

// ../../packages/core/dist/patterns/data/builtin-exempt/claude.json
var claude_default = [
  {
    name: "ToolSearch",
    reason: "grade 1 (conversation) \u2014 tool discovery, no workspace/exec reach"
  },
  {
    name: "TodoWrite",
    reason: "grade 1 (conversation) \u2014 session plan tracking"
  },
  {
    name: "TodoRead",
    reason: "grade 1 (conversation) \u2014 session plan tracking"
  ...
  {
    name: "TaskCreate",
    reason: "grade 1 (conversation) \u2014 sub-task bookkeeping"
  },
  {
    name: "TaskUpdate",
    reason: "grade 1 (conversation) \u2014 sub-task bookkeeping"
  ...
  },
  {
    ...
    reason: "grade 1 (conversation) \u2014 prompts the user, no side effects"
  },
  {
    name: "EnterPlanMode",
    reason: "grade 1 (conversation) \u2014 plan-mode toggle"
  },
  {
    name: "ExitPlanMode",
    reason: "grade 1 (conversation) \u2014 plan-mode toggle"
  ...
  {
    name: "exit_plan_mode",
    reason: "grade 1 (conversation) \u2014 plan-mode toggle (legacy snake_case wire alias)"
  }
];

// ../../packages/core/dist/patterns/data/builtin-exempt/codex.json
var codex_default = [
  {
    name: "view_image",
    reason: "grade 2 (workspace read) \u2014 renders an image file to the user"
  },
  {
    name: "update_plan",
    reason: "grade 1 (conversation) \u2014 session plan tracking"
  ...
  },
  {
    ...
    reason: "grade 2 (workspace read) \u2014 read-only MCP resource enumeration"
  },
  {
  ...
  },
  {
    ...
    reason: "grade 2 (workspace read) \u2014 read-only MCP resource fetch"
  },
  {
    name: "get_goal",
    reason: "grade 1 (conversation) \u2014 session goal bookkeeping"
  },
  {
    name: "create_goal",
    reason: "grade 1 (conversation) \u2014 session goal bookkeeping"
...
// ../../packages/core/dist/patterns/data/builtin-exempt/cursor.json
var cursor_default = [
  { name: "Read", reason: "grade 2 (workspace read) \u2014 file read" },
  { name: "Grep", reason: "grade 2 (workspace read) \u2014 text search" },
  { name: "Glob", reason: "grade 2 (workspace read) \u2014 file-name search" },
  {
    name: "SemanticSearch",
    reason: "grade 2 (workspace read) \u2014 semantic search"
  },
  {
    name: "SwitchMode",
    reason: "grade 1 (conversation) \u2014 agent mode toggle"
  },
  {
    name: "TodoWrite",
    reason: "grade 1 (conversation) \u2014 session plan tracking"
  ...
  {
    name: "todo_write",
    reason: "grade 1 (conversation) \u2014 session plan tracking (snake_case wire alias)"
  },
  {
    name: "AskQuestion",
    reason: "grade 1 (conversation) \u2014 prompts the user, no side effects"
  ...
  {
    name: "ask_question",
    reason: "grade 1 (conversation) \u2014 prompts the user (snake_case wire alias)"
  },
  {
    name: "Task",
    reason: "grade 1 (conversation) \u2014 sub-agent bookkeeping; sub-agent tool calls hook individually"
  },
  {
    name: "Await",
    reason: "grade 1 (conversation) \u2014 async sub-task wait, no side effects"
  }
];

// ../../packages/core/dist/patterns/guard-tool-names.generated.js
var GUARD_TOOL_NAMES = /* @__PURE__ */ new Set([
...
var GUARD_META_TOOL_NAMES = /* @__PURE__ */ new Set([
  "tc_create_stepup_session",
  "tc_inspect_stepup_state",
  "tc_poll_stepup_session",
  "tc_poll_stepup_session_wait"
...
// ../../packages/core/dist/patterns/tool-rules.js
var GUARD_PROVIDERS = [
  "claude",
  "codex",
  "cursor",
...
var TRANSCODES_NS_REGEX = /^(plugin_)?mcp_plugin_transcodes_guard$/;
...
function isBuiltinExemptToolName(provider, toolName) {
  if (provider === void 0)
    return false;
...
async function request(config, input) {
  const path4 = input.path.startsWith("/") ? input.path : `/${input.path}`;
  ...
  const url = `${config.apiBaseV1}${path4}${qs ? `?${qs}` : ""}`;
  const headers = {
...
// ../../packages/core/dist/stepup/console.js
import { spawn } from "child_process";

// ../../packages/core/dist/stepup/session.js
...
async function createStepupSession(config, args) {
  ...
  return {
    envelope,
    sid: payload ? readString3(payload, "sid") : void 0,
    browserUrl: payload ? readString3(payload, "url") ?? readString3(payload, "browser_url") ?? readString3(payload, "browserUrl") : void 0,
    expiresAt: payload ? readString3(payload, "expiresAt") ?? readString3(payload, "expires_at") : void 0,
    mode: payload ? readString3(payload, "mode") : void 0
  };
}
...
async function pollStepupSession(config, sid) {
  ...
  return {
    envelope,
    status: payload ? readString3(payload, "status") : void 0
  };
}
async function pollStepupByCoordinate(config, coordinate) {
  ...
  const payload = readStepupPayload(envelope);
  const sid = payload ? readString3(payload, "sid") : void 0;
  return {
    envelope,
    status: payload ? readString3(payload, "status") : void 0,
    ...sid ? { sid } : {}
...
async function pollStepupSessionWait(config, target, options = {}) {
  const normalized = typeof target === "string" ? { sid: target } : {
    sid: target.sid?.trim() || void 0,
    resource: target.resource?.trim() || void 0,
    action: target.action?.trim() || void 0
  };
  ...
  const maxWaitMs = options.maxWaitMs ?? 3e5;
  ...
  while (true) {
    attempts += 1;
    const result = normalized.resource && normalized.action && !normalized.sid ? await pollStepupByCoordinate(config, {
      resource: normalized.resource,
      action: normalized.action
...
// ../../packages/core/dist/stepup/token-store.js
...
// ../../packages/core/dist/stepup/gate.js
...
// ../../packages/core/dist/stepup/inspector.js
...
function parseMemberAccessToken(rawToken) {
  ...
  const organizationId = readString4(payload, "oid");
  const projectId = readString4(payload, "pid");
  const memberId = readString4(payload, "mid");
  ...
  return {
    ...
    claims: {
      ...
      aud,
      exp,
      iss: readString4(payload, "iss"),
      jti: readString4(payload, "jti"),
...
// ../../packages/core/dist/stepup/rbac-check.js
var FAILURE_MESSAGE_MAX_LENGTH = 240;
function sanitizeFailureText(text) {
  ...
  return flat.length > FAILURE_MESSAGE_MAX_LENGTH ? `${flat.slice(0, FAILURE_MESSAGE_MAX_LENGTH)}\u2026` : flat;
}
function extractFailureMessage(data) {
  if (!data || typeof data !== "object")
    return void 0;
  const o = data;
  const text = typeof o.message === "string" && o.message.trim() ? o.message.trim() : Array.isArray(o.message) && o.message.length > 0 && o.message.every((m) => typeof m === "string") ? o.message.join("; ") : typeof o.error === "string" && o.error.trim() ? o.error.trim() : void 0;
  const logId = typeof o.logId === "string" && o.logId ? o.logId : void 0;
  const combined = text && logId ? `${text}; logId=${logId}` : text ?? (logId ? `logId=${logId}` : void 0);
  return combined ? sanitizeFailureText(combined) : void 0;
}
async function evaluateAction(config, body) {
  ...
  if (typeof resource !== "string" || typeof action !== "string")
    return malformed;
  const status = p.status === "pending" || p.status === "verified" || p.status === "rejected" ? p.status : null;
  const summary = typeof p.summary === "string" && p.summary.trim() ? p.summary.trim() : "";
  const provider = typeof p.provider === "string" && GUARD_PROVIDERS.includes(p.provider) ? p.provider : null;
  return {
    ok: true,
    verdict: {
      permission,
...
var CLI_INSTALL_HINT_WIN = `Set-ExecutionPolicy Bypass -Scope Process -Force; irm ${CLI_INSTALL_PS1_URL} | iex; transcodes install`;
var APP_CONSOLE_RBAC_LOCATION = "https://app.transcodes.io \u2192 RBAC \u2192 Roles";
function cliInstallBlurb() {
  return `Install the CLI (macOS/Linux: \`${CLI_INSTALL_HINT_UNIX}\`; Windows PowerShell: \`${CLI_INSTALL_HINT_WIN}\`).`;
}
...
function formatNoTokenReason(block) {
  return `Bash blocked by transcodes-guard: ${block.reason}. Step-up MFA gate is not configured (no Transcodes token found). Tell the user to ${cliInstallBlurb()} Run \`transcodes login\`, sign in with Google, and choose an organization. Or run the command outside the agent.`;
}
...
function formatBlockByPolicyReason(decision) {
  return appendBackendReasoning(`Blocked by transcodes-guard: ${decision.block.reason}. Your RBAC role denies this action (resource="${decision.resource}", action="${decision.action}") \u2014 permission level 0 is a hard deny; step-up MFA only unlocks actions already at level 2. Report this to the user; do not retry. An admin must grant the permission at ${APP_CONSOLE_RBAC_LOCATION}. Do not use get_console_url or auth.transcodes.io for RBAC edits.`, decision.reasoning);
}
function formatBlockByPolicySystemMessage(decision) {
...
function formatStepupCreateFailedDetail(decision) {
  const { failure } = decision;
  return failure.reason === "no-token" ? `No Transcodes token found \u2014 step-up MFA gate is unavailable. ${cliInstallBlurb()} Run \`transcodes login\`, sign in with Google, and choose an organization.` : failure.reason === "create-failed" ? `Step-up MFA session could not be started${failure.detail ? ` (${failure.detail})` : ""}.` : `Step-up MFA gate errored${failure.detail ? ` (${failure.detail})` : ""}.`;
}
function formatStepupCreateFailedReason(decision) {
  return `Bash blocked by transcodes-guard: ${decision.block.reason}. ${formatStepupCreateFailedDetail(decision)} Report the failure to the user; do not retry until step-up is available.`;
}
function formatStepupCreateFailedSystemMessage(decision) {
...
function formatStepupChallengedReason(decision) {
  return `Step-up MFA pending. ${decision.resource}:${decision.action}. Open ${decision.browserUrl}, complete WebAuthn, then call MCP tool \`tc_poll_stepup_session_wait\` with resource="${decision.resource}" action="${decision.action}" (or sid="${decision.sid}") and retry the same Bash command.`;
}
function formatStepupChallengedSystemMessage(decision) {
  const launchLine = decision.browserLaunched ? "A browser tab has been opened automatically:" : "A browser tab was already opened for this session \u2014 reuse it, or open:";
  return appendBackendReasoning([
    ...
    `  2. Immediately call the MCP tool \`tc_poll_stepup_session_wait\` with resource="${decision.resource}" and action="${decision.action}" (sid optional). It waits up to ~5 min (session TTL) until verified or timeout.`,
    '  3. On `outcome: "verified"` retry the SAME Bash command \u2014 the hook detects the verified state and allows it. On `outcome: "timeout"`, `outcome: "rejected"`, or `outcome: "not_found"`: tell the user in one short line that this command did not run; skip the blocked command; continue other work. Do NOT re-poll, reopen auth tabs, or retry the SAME blocked command unless the user explicitly asks to authenticate again. Do NOT invent an alternate command that works around the blocked action.',
    "  4. If the user says stop/cancel/skip at any time, abort this command immediately and continue other work \u2014 do not keep waiting."
  ].join("\n"), decision.reasoning);
}
function formatStepupRejectedReason(decision) {
  return `Step-up MFA was declined. Tell the user this command did not run, skip it, and continue other work. Do not retry or poll unless they explicitly ask to authenticate again (${decision.resource}/${decision.action}).`;
}
function formatPollStepupSessionWaitAgentContext() {
  return [
...
function formatStepupRejectedSystemMessage(decision) {
  return appendBackendReasoning([
    ...
    "  1. Tell the user in one short line that step-up MFA was declined and this command did not run.",
    "  2. Do NOT call `tc_poll_stepup_session_wait` or `tc_poll_stepup_session` \u2014 the challenge is terminal for this command.",
    "  3. Skip this blocked command. Do NOT retry it or invent an alternate command that works around the block.",
    "  4. Continue other unrelated work in the plan. Only start step-up again for this command if the user explicitly asks to authenticate and retry.",
    "",
...
// ../../packages/core/dist/stepup/config.js
...
function loadStepupConfig() {
  ...
  if (!tokenRaw) {
    throw new Error(`No Transcodes token found. Install the CLI (macOS/Linux: \`${CLI_INSTALL_HINT_UNIX}\`; Windows: \`${CLI_INSTALL_HINT_WIN}\`), then run \`transcodes login\` to sign in and choose an organization.`);
  }
...
// ../../packages/core/dist/stepup/evaluate.js
...
function wireToolName(input) {
  return input.toolName !== "Unknown" ? input.toolName : void 0;
}
...
function classifyToolCall(input, provider) {
  const name = wireToolName(input);
  if (name && (isGuardMetaToolName(name) || isBuiltinExemptToolName(provider, name))) {
    return null;
  }
...
async function evaluatePreToolUse(input) {
  ...
  try {
    ...
    if (result.ok) {
      ...
      failureDetail = result.kind === "network" ? `backend unreachable${result.message ? `: ${result.message}` : " (network/timeout)"}` : result.kind === "http" ? `backend evaluate failed: HTTP ${result.status}${result.message ? ` \u2014 ${result.message}` : ""}` : "malformed backend response";
    }
  } catch (err) {
    verdict = null;
    failureReason = "error";
  ...
  if (!verdict?.sid || !verdict.url) {
    return {
      ...
      failure: {
        ...
        detail: failureDetail ?? "backend returned permission=2 without a session (sid/url missing)"
      },
      reasoning: backendReasoning
    };
  }
  ...
  const pending = verdict.status === "pending" || verdict.status === null || verdict.status === void 0;
  const reused = verdict.exist === true;
  let browserLaunched = false;
  if (pending && !reused) {
    openBrowser2(verdict.url);
...
// ../../packages/core/dist/contract/noop.js
...
// ../../packages/core/dist/contract/registry.js
var current = null;
...
// ../../packages/core/dist/contract/tool-def.js
function defineTool(def) {
  return def;
}
function registerToolDefinitions(server, defs, wrapProtected) {
  for (const def of defs) {
    let handler;
    if (def.stepUp !== void 0) {
      if (wrapProtected === void 0) {
...
// ../../packages/gate-backend/dist/mcp-tools/define.js
function defineBackendTool(def) {
  return def;
}
function defineProtectedBackendTool(def) {
  return def;
}
...
// ../../packages/gate-backend/dist/mcp-tools/transcodes-client.js
var ENDPOINT_MAP = {
  // Project
  get_project: "/project",
  // Audit
...
async function reqEnvelope(config, input, toolName, pathSuffix) {
  ...
  const path4 = pathSuffix ? `${base}${pathSuffix}` : base;
  return request(config, { ...input, path: path4 });
}
async function req(config, input, toolName, pathSuffix) {
  ...
  const path4 = pathSuffix ? `${base}${pathSuffix}` : base;
  const envelope = await request(config, { ...input, path: path4 });
...
// ../../packages/gate-backend/dist/mcp-tools/auth-devices.js
var authDeviceToolDefinitions = [
  defineBackendTool({
...
// ../../packages/gate-backend/dist/mcp-tools/jwk.js
...
// ../../packages/gate-backend/dist/mcp-tools/members.js
...
var memberToolDefinitions = [
  ...
  defineProtectedBackendTool({
    ...
    description: "Temporarily SUSPEND a member: blocks login and invalidates active sessions. Reversible via unsuspend_member. Verified action \u2014 step-up MFA enforced by the backend StepUpSessionGuard on the API call. " + MEMBER_SUSPENSION_API_NOTE,
  ...
  defineProtectedBackendTool({
    ...
    description: "Lift a member's suspension and restore their ability to log in and create sessions. Use only on members previously suspended. Verified action \u2014 step-up MFA enforced by the backend StepUpSessionGuard on the API call. " + MEMBER_SUSPENSION_API_NOTE,
  ...
  defineProtectedBackendTool({
    ...
    title: "Create member",
    description: "Create a member (CreateMemberDto). member_id/name may be auto-generated. Use for onboarding or manual provisioning. RBAC-gated by the backend StepUpSessionGuard (0=block, 1=allow, 2=step-up MFA). Auth: TRANSCODES_TOKEN sent as x-transcodes-token (not in body).",
  ...
  defineProtectedBackendTool({
    ...
    title: "Update member",
    description: "Update member PROFILE fields \u2014 name, email, metadata (UpdateMemberDto, flat shape). RBAC-gated by the backend StepUpSessionGuard (0=block, 1=allow, 2=step-up MFA). member_id is required \u2014 supply the target member explicitly (it may differ from the caller). To REASSIGN a member's ROLE, use `update_member_role` instead: it validates the role exists (this tool writes `role` straight through with no validation). Prefer omitting `role` here.",
...
// ../../packages/gate-backend/dist/mcp-tools/membership.js
var membershipToolDefinitions = [
  defineBackendTool({
  ...
  defineBackendTool({
    ...
    description: "Returns the resource limits per plan tier. Product features are unlimited on every plan. The first 2 accepted organization members are free. From the 3rd member, every member is billed at price_per_seat (Standard: $9 / member / month; 3 members = $27). A card is required before inviting a teammate. Use this to build pricing comparison UI or explain seat billing.",
...
// ../../packages/gate-backend/dist/mcp-tools/meta.js
var INSTRUCTIONS_URL = "https://transcodes.io/instructions";
var metaToolDefinitions = [
  defineBackendTool({
  ...
  defineBackendTool({
    ...
    description: "Mint a step-up-protected Open Console URL (auth.transcodes.io). Open Console access is gated behind step-up MFA via POST .../console/session; this tool returns the browser URL the user must visit to authenticate (WebAuthn) before reaching Open Console. Use when the user needs to perform browser-only actions: passkey register/update/revoke, authenticator register/update/revoke, TOTP enroll/update/revoke, OTP flows, JWK backup, or subscription portal (cancel, payment method, invoices). RBAC edits are App Console only (https://app.transcodes.io \u2192 RBAC \u2192 Roles), not this URL. Direct the user to visit the returned browser_url and complete the action there.",
    summary: "Mint an Open Console URL (auth.transcodes.io) for passkeys, TOTP, OTP, JWK backup, and billing. RBAC edits are App Console only (https://app.transcodes.io).",
    category: "Meta & Identity",
    access: "api",
    mutating: false,
    meta: false,
  ...
  defineBackendTool({
    ...
    handler: async ({ topic }) => {
      ...
      try {
        ...
        if (trimmed) {
          return textResult(JSON.stringify({ topic: trimmed, instructions: content }, null, 2));
        }
        return textResult(content);
      } catch (err) {
        return textResult(`Could not fetch the integration guide: ${err instanceof Error ? err.message : String(err)}`, true);
      } finally {
        clearTimeout(timer);
      }
    }
...
var organizationToolDefinitions = [
  defineBackendTool({
    ...
    summary: "Returns the currently authenticated platform user (Firebase/console account).",
...
// ../../packages/gate-backend/dist/mcp-tools/passcode.js
var passcodeToolDefinitions = [
  defineProtectedBackendTool({
    name: "tc_passcode_create",
    title: "Create recovery passcode",
    description: "Create a recovery passcode (CreatePasscodeDto in body). RBAC-gated by the backend StepUpSessionGuard (0=block, 1=allow, 2=step-up MFA). Use for onboarding, support, or admin provisioning.",
...
// ../../packages/gate-backend/dist/mcp-tools/project.js
...
// ../../packages/gate-backend/dist/mcp-tools/rbac.js
...
var rbacToolDefinitions = [
  ...
  defineProtectedBackendTool({
    name: "tc_retire_role",
    title: "Retire role",
    description: "Retire a role from the project. Use when the user wants to remove, drop, or discard a role. Verified action \u2014 step-up MFA enforced by the backend StepUpSessionGuard on the API call. Body { project_id } is injected from TRANSCODES_TOKEN by the server.",
  ...
  defineProtectedBackendTool({
    ...
    description: "Set per-resource permission matrix for a role. 0=deny, 1=allow, 2=allow+step-up. Requires the caller MAT role to have system/update >= 1; calls at level 0 are denied. If denied, an admin must edit RBAC at https://app.transcodes.io \u2192 RBAC \u2192 Roles. Verified action \u2014 step-up MFA enforced by the backend StepUpSessionGuard on the API call.",
  ...
  defineProtectedBackendTool({
    ...
    description: "Change a member's assigned role (UpdateMemberRoleDto) \u2014 the canonical role-reassignment path. Validates the target role EXISTS in the project before assigning (unlike `update_member`, which writes `role` unchecked). Use this whenever the user wants to change a member's role. Requires the caller MAT role to have system/update >= 1; calls at level 0 are denied. If denied, an admin must edit RBAC at https://app.transcodes.io \u2192 RBAC \u2192 Roles. Verified action \u2014 step-up MFA enforced by the backend StepUpSessionGuard on the API call.",
  ...
  defineProtectedBackendTool({
    ...
    description: "Retire a resource key from the project. Use when the user wants to remove, drop, or discard a resource. Verified action \u2014 step-up MFA enforced by the backend StepUpSessionGuard on the API call. Path: resource_key. Query: project_id. No JSON body.",
  ...
  defineProtectedBackendTool({
    name: "tc_create_role",
    title: "Create role",
    description: "Create a new role (CreateRoleDto). Use before set_role_permissions to fill per-resource access. RBAC-gated by the backend StepUpSessionGuard (0=block, 1=allow, 2=step-up MFA). " + PROJECT_ID_GUIDANCE,
  ...
  defineProtectedBackendTool({
    name: "tc_update_role",
    title: "Update role",
    description: "Update role metadata (UpdateRoleDto). RBAC-gated by the backend StepUpSessionGuard (0=block, 1=allow, 2=step-up MFA). " + PROJECT_ID_GUIDANCE,
  ...
  defineProtectedBackendTool({
    ...
    description: "Add a new resource key (CreateResourceDto). Every existing role is initialized with the default permission matrix for the new key: read = allow (1), and create/update/delete = allow + step-up MFA (2). RBAC-gated by the backend StepUpSessionGuard (0=block, 1=allow, 2=step-up MFA). " + PROJECT_ID_GUIDANCE,
  ...
  defineProtectedBackendTool({
    ...
    description: "Update resource label/description (UpdateResourceDto). Key stays the same. RBAC-gated by the backend StepUpSessionGuard (0=block, 1=allow, 2=step-up MFA). " + PROJECT_ID_GUIDANCE,
...
// ../../packages/gate-backend/dist/mcp-tools/definitions.js
var backendToolDefinitions = [
...
// ../../packages/gate-backend/dist/mcp-tools/rbac-validate.js
...
// ../../packages/gate-backend/dist/mcp-tools/stepup-helper.js
...
function stepupRequiredResult(def, backendMessage) {
  return textResult2({
    ...
    message: backendMessage ?? "Step-up MFA is required before running this protected MCP tool.",
    tool: def.name,
...
function rbacDeniedResult(def, backendMessage) {
  return textResult2({
    ...
    blocked: true,
    code: "RBAC_DENIED",
    message: backendMessage ?? `This action is denied by the project's RBAC policy (${def.stepUp.resource}/${def.stepUp.action} = 0).`,
    tool: def.name,
    resource: def.stepUp.resource,
    action: def.stepUp.action,
    next_actions: [
...
function rbacUnresolvedResult(def, backendMessage) {
  return textResult2({
    ...
    message: backendMessage ?? "The backend could not resolve the RBAC permission for this action.",
    tool: def.name,
    resource: def.stepUp.resource,
    action: def.stepUp.action,
    next_actions: [
...
function extractErrorCode(data) {
  ...
  return typeof errorCode === "string" && errorCode.length > 0 ? errorCode : void 0;
}
function wrapProtectedTool(def) {
  return async (args) => {
    const config = loadStepupConfig();
