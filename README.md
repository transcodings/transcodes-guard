...
On the CLI, `agy plugin list` should then show `transcodes-guard`. Also save your token — recommended: `npm install -g @bigstrider/transcodes-cli` then `transcodes` (dashboard). Non-interactive: `transcodes set <token> -l <label>`. Config-less envs (CI): export `TRANSCODES_TOKEN`.
