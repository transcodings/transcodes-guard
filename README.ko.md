...
**1단계 — Codex 마켓플레이스로 설치.** 저장소는 `./plugins/codex`를 가리키는 Codex 카탈로그인 `.agents/plugins/marketplace.json`을 제공합니다. 저장소를 클론하고 커밋된 `dist/`를 빌드한 뒤, 저장소 루트를 마켓플레이스로 등록하고 설치하세요.
...
**3단계 — 토큰 저장**(멤버 MCP JWT). 권장: `npm install -g @bigstrider/transcodes-cli` 후 `transcodes`를 실행하면 로컬 대시보드가 열립니다(터미널에 URL 출력, 기본 포트 3847) — 거기에 토큰을 붙여넣으세요(`~/.transcodes/config.json`에 저장되어 모든 세션이 읽음). 비대화형: `transcodes set <token> -l <label>`. config 파일이 없는 환경(CI)에서는 `TRANSCODES_TOKEN`을 export 하세요 — 저장된 토큰이 없을 때만 쓰이는 fallback입니다. 셋 다 없으면 hook이 위험 명령을 여전히 DENY 하지만 step-up 세션은 열지 못합니다.
...
최초 실행 시 Cursor가 일회성 hook 신뢰 검토를 띄웁니다(커맨드 팔레트 → "Cursor: Review Hooks"). 토큰도 저장하세요 — 권장: `npm install -g @bigstrider/transcodes-cli` 후 `transcodes`(대시보드). 비대화형: `transcodes set <token> -l <label>`. config 파일이 없는 환경(CI)에서는 Cursor를 실행하는 셸에서 `TRANSCODES_TOKEN`을 export 하세요.
...
CLI에서 `agy plugin list`를 실행하면 `transcodes-guard`가 표시됩니다. 토큰도 저장하세요 — 권장: `npm install -g @bigstrider/transcodes-cli` 후 `transcodes`(대시보드). 비대화형: `transcodes set <token> -l <label>`. config 파일이 없는 환경(CI)에서는 `TRANSCODES_TOKEN`을 export 하세요.
...
step-up이 실제로 시작되려면 토큰(멤버 MCP JWT)이 필요합니다. 권장: CLI 설치(`npm install -g @bigstrider/transcodes-cli`) 후 `transcodes`를 실행해 대시보드에서 입력하세요. 비대화형: `transcodes set <token> -l <label>`. config 파일이 없는 환경(CI)에서는 `TRANSCODES_TOKEN`을 export 하세요 — 저장 파일에 토큰이 없을 때만 쓰이는 fallback입니다.
