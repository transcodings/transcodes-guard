...
- **`tasks`** — 에이전트가 무슨 일을 하던 중이었는지 한 줄로 요약한 값. 호스트가 훅에 알려 준 transcript를 로컬에서 읽어 만듭니다. 호스트의 세션 제목과 가장 최근 지시의 발췌로 이루어지며, 각각 300자로 잘립니다.

transcript 파일 자체는 머신 밖으로 나가지 않고 요약만 나갑니다. 다만 짚어 둘 점이 있습니다. **게이트 대상 호출마다 프롬프트 발췌가 전송됩니다.** 프롬프트에 붙여넣은 비밀값도 함께 나갈 수 있으니, 백엔드로 보내는 다른 데이터와 똑같이 다루세요. 클라이언트가 디스크에 기록하는 것은 없습니다(`inspect_stepup_state`의 빈 응답이 곧 그 답입니다).
