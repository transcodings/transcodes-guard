...
게이트 대상 호출에서 훅이 밖으로 내보내는 요청은 `POST /guard/evaluate` 하나뿐이며, 다음을 싣습니다.

- 호스트의 훅 페이로드 원문(tool 이름과 인자)과 작업 디렉터리
- 세션·턴·tool 호출에 대한 호스트 자신의 식별자, 그리고 에이전트를 구동하는 모델
- **`tasks`** — 현재 사용자 프롬프트를 300자 이내의 한 줄로 요약한 값. prompt hook이 저장한 현재 턴 캐시를 우선하고, 없으면 호스트 transcript를 최선 노력 폴백으로 사용합니다. 폴백 요약에는 호스트의 세션 제목이 추가될 수 있습니다.

transcript와 원문 캐시 파일은 머신 밖으로 나가지 않고 요약만 나갑니다. 다만 짚어 둘 점이 있습니다. **게이트 대상 호출마다 프롬프트 발췌가 전송됩니다.** 프롬프트에 붙여넣은 비밀값도 함께 나갈 수 있으니, 백엔드로 보내는 다른 데이터와 똑같이 다루세요. prompt와 tool 이벤트를 연결하기 위해 클라이언트는 세션당 원문 프롬프트 최대 4개를 24시간 동안 보관합니다(각 32 KiB, 전체 세션 파일 256개). 경로는 호스트의 `PLUGIN_DATA`/`CLAUDE_PLUGIN_DATA`, source/dev 실행에서는 `~/.transcodes/cache/prompts`이며 디렉터리는 `0700`, 파일은 `0600`, 세션 id는 해시 파일명으로만 사용됩니다. 이 telemetry 캐시는 신뢰 게이트 상태와 분리되므로 `inspect_stepup_state`는 계속 후자만 보고합니다.
