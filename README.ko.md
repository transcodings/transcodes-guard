...
Codex는 legacy `.claude-plugin/marketplace.json`보다 `.agents/plugins/marketplace.json`을 **우선** 해석하므로, 항상 Claude용이 아닌 Codex 플러그인(`./plugins/codex`)을 설치합니다. 특정 버전을 고정하려면 `codex plugin marketplace add transcodings/transcodes-guard --ref transcodes-guard-v0.25.0`을 쓰세요.
