...
export function readPending(fp) {
    if (!fp)
...
export function listFpPendings() {
    const out = [];
 ...
 */
export function findPendingBySid(sid) {
...
export function firstInFlightFpPending(now = Date.now()) {
    for (const rec of listFpPendings()) {
 ...
 * Bash/user record.
 */
export function firstActivePending(now = Date.now()) {
    const global = readPending();
 ...
 */
export function sweepStepup(now = Date.now()) {
    for (const rec of listFpPendings()) {
        const fp = rec.fp;
        if (!fp)
            continue;
