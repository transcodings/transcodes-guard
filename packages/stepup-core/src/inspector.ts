...
function inspectVerifiedFile(file: string, now: number, fp?: string): VerifiedInspection {
...
function inspectPendingFile(file: string, now: number, fp?: string): PendingInspection {
  const data = readJsonFile(file);
  if (!data) return { exists: false };
  const sid = typeof data.sid === "string" ? data.sid : null;
  const status =
    data.status === "verified" || data.status === "pending" ? data.status : null;
  const createdAt =
    typeof data.createdAt === "number" ? data.createdAt : null;
  const command = typeof data.command === "string" ? data.command : null;
  const browserUrl =
    typeof data.browserUrl === "string" ? data.browserUrl : "";
