 ...
 */
export function getPromptSessionId(now = Date.now()) {
 ...
 */
export function rotatePromptSession(now = Date.now()) {
    const next = { id: mintId(), createdAt: now };
    writeRecord(next);
    return next.id;
}
/** Explicit lock: drop the bucket so the next command starts a fresh window. */
export function clearPromptSession() {
    try {
...
//# sourceMappingURL=prompt-session.js.map