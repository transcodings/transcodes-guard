...
export function policyBundleSha384(body) {
 ...
 */
export function verifyAndParsePolicyBundle(raw) {
    ...
    if (!parsed.success) {
        throw new PolicyBundleError(`bundle schema invalid: ${parsed.error.issues
            .map((i) => `${i.path.join('.')}: ${i.message}`)
            .join('; ')}`);
    }
    return parsed.data;
}
export function policyBundleCachePath(projectId) {
 ...
 */
export function readCachedPolicyBundle(projectId, ttlMs = POLICY_BUNDLE_TTL_MS) {
    let raw;
    try {
...
export function writeCachedPolicyBundle(projectId, bundle) {
 ...
 */
export async function fetchPolicyBundle(config, currentRevision) {
 ...
 */
export async function refreshPolicyBundle(config, opts = {}) {
    try {
 ...
 */
export function loadEffectivePatterns() {
