...
export const membershipToolDefinitions: readonly GuardToolDefinition[] = [
  ...
  defineBackendTool({
    ...
    description:
      ...
      'From the 3rd member, every member is billed at price_per_seat (Standard: $9 / member / month; 3 members = $27). ' +
  ...
  defineBackendTool({
    ...
    handler: async () => {
      ...
      const text = await req(
        config,
        { method: 'GET', query: { organization_id: config.organizationId } },
