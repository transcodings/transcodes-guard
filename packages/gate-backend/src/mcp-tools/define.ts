...
export function defineBackendTool<S extends ZodRawShape>(
  def: Omit<PlainToolDefinition, 'inputSchema' | 'handler' | 'meta'> & {
    meta: false;
    inputSchema: S;
...
export function defineProtectedBackendTool<S extends ZodRawShape>(
  def: Omit<ProtectedToolDefinition, 'inputSchema' | 'run' | 'meta'> & {
    meta: false;
    inputSchema: S;
    run: (
      config: StepupConfig,
