...
export function registerMemberTools(server: McpServer): void {
  ...
  server.registerTool(
    'get_member_suspension',
    {
      ...
      description:
        'Check whether a member is currently suspended and when it was applied. Returns { revoked_at: ISO date string } if suspended, or { revoked_at: null } if active. Read-only. ' +
  ...
  server.registerTool(
    ...
    async ({ body }) => {
      ...
      return execProtectedTool('create_member', (sid) =>
        req(config, { method: 'POST', body: { ...body, project_id: config.projectId }, stepUpSid: sid }, 'create_member')
      );
    }
  );
  ...
  server.registerTool(
    ...
    async ({ body }) => {
      ...
      return execProtectedTool('update_member', (sid) =>
        req(config, { method: 'PUT', body: { ...body, project_id: config.projectId }, stepUpSid: sid }, 'update_member')
      );
    }
  );
}
