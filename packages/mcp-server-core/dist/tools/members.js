...
export function registerMemberTools(server) {
    ...
    server.registerTool('create_member', {
        ...
        const config = loadStepupConfig();
        return execProtectedTool('create_member', (sid) => req(config, { method: 'POST', body: { ...body, project_id: config.projectId }, stepUpSid: sid }, 'create_member'));
    });
    server.registerTool('update_member', {
        ...
        const config = loadStepupConfig();
        return execProtectedTool('update_member', (sid) => req(config, { method: 'PUT', body: { ...body, project_id: config.projectId }, stepUpSid: sid }, 'update_member'));
    });
}
