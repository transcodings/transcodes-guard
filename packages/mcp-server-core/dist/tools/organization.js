...
const MSG_PLATFORM_CONSOLE = "User and organization management must be done in the Transcodes console. This MCP tool does not call the API.";
const MSG_ORG_CONSOLE = "Organization settings, user invitations, and invitation management (send, update, cancel, accept, decline) must be done directly in the Transcodes console at https://transcodes.io. This MCP tool does not call the API.";
const MSG_MEMBER_TOKEN_CONSOLE = "Per-member MCP tokens (TRANSCODES_TOKEN — the JWT sent as the x-transcodes-token header) can only be issued from the Transcodes console at https://app.transcodes.io. " +
...
export function registerOrganizationTools(server) {
    ...
    server.registerTool("member_token_create", {
        ...
        description: "Blocked: issuing a per-member MCP token (TRANSCODES_TOKEN — the JWT used as x-transcodes-token) must be done in the Transcodes console only. " +
