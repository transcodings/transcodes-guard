...
export function coreToolDefinitions(
  ...
  return [
    ...
    defineTool({
      ...
      description:
        'Block until the step-up session reaches `verified`, `rejected`, `not_found`, or ' +
      ...
      inputSchema: {
        ...
        max_wait_ms: z
          ...
          .optional()
          .describe(
            'Maximum time to wait in ms. Defaults to 300_000 (5 min, Redis TTL).',
          ),
        interval_ms: z
          .number()
          .int()
    ...
    defineTool({
      ...
      description:
        'Reports what step-up state this client holds: nothing. Guard v3 ' +
    ...
    defineTool({
      ...
      handler: async ({ command, cwd, tool_name, tool_input }) => {
        ...
        const { stdout, stderr, exitCode } = await new Promise<{
          ...
          child.stdout.on(
            'data',
            (b: Buffer) => (stdout += b.toString('utf8')),
          );
          child.stderr.on(
            'data',
            (b: Buffer) => (stderr += b.toString('utf8')),
          );
          child.on('close', (code) =>
        ...
        const stepUpTag =
          /^transcodes-guard: STEPUP-CHALLENGED sid=(\S+)/m.exec(stderr);
