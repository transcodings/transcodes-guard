...
- The long-lived member token lives in `~/.transcodes/config.json`, **not** the cache dir — so it survives cache cleanup and is discoverable by the MCP server plus four hook subprocesses that don't inherit a GUI host's shell env. `resolveToken` precedence is strictly file → env `TRANSCODES_TOKEN` → `null` (the CLI-written file wins; env is only a fallback for config-less envs). Adding a token requires a mandatory label; readers never throw on a malformed file (return `null`/empty).
