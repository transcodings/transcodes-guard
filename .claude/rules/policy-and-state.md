...
- **Embed system rule JSON via static import** with `with { type: 'json' }` (`./data/danger-patterns.json`, `./data/tool-rules.json`) — never read it at runtime, and keep the JSON under `src/patterns/data/` (the build copies it to `dist/patterns/data/`).
