...
- Everything else reaches the backend through `getGateBackend()` (from `@transcodes-guard/core/contract`). When no backend is registered it returns `denyByDefaultBackend` (`gate-contract/src/noop.ts`): its **hook** methods no-op/pass, but its **server-call** methods *throw* `gate backend not installed`. This is deliberate — a backend-less build type-checks and is provably never shipped as functional.
