...
- `build:plugin` runs a `prebuild:plugin` step that **generates committed source**: `scripts/stamp-build-info.mjs` stamps the version into `packages/core/src/build-info.ts` (src root — shared by `server/` and `stepup/` via `../build-info.js`) + `plugins/*/src/version.ts`, and `scripts/generate-router-files.mjs` renders the `/transcodes` body from the single source `scripts/router-body.mjs` into `packages/core/src/server/router-body.ts` + the four per-host command/skill files. All are committed source (not gitignored); hand-edits are overwritten each build. CI runs `generate-router-files.mjs --check` **before** `build:plugin` (the build's `prebuild:plugin` would otherwise regenerate these files and the check would compare them against themselves), so committed router drift fails the build.
...
- Freshness is guarded only by: (a) `build:plugin` succeeding, (b) the **gitignore-existence guard**, (c) the **22 smoke tests** (claude-code 8 + codex 3 + antigravity 5 + cursor 6), written to Guard v3 semantics (no-token/backend-unreachable fail-closed, hook-skip, latch-based Stop — no on-disk verified/pending fixtures).
