...
- `build:plugin` runs a `prebuild:plugin` step that **generates committed source**: `scripts/stamp-build-info.mjs` stamps the version into `packages/core/src/build-info.ts` (src root — shared by `server/` and `stepup/` via `../build-info.js`) + `plugins/*/src/version.ts`, and `scripts/generate-router-files.mjs` (run under tsx: `node --import tsx …` — `scripts/tool-metadata.mts` imports the TS tool definition sources) renders every tool-definition-derived artifact: `packages/core/src/server/router-body.ts`, `packages/core/src/patterns/guard-tool-names.generated.ts` (`GUARD_TOOL_NAMES` / meta set / protected rules), `cli/src/tool-catalog.generated.ts`, and the four per-host command/skill files. All are committed source (not gitignored); hand-edits are overwritten each build. CI runs the `--check` mode **before** `build:plugin` (the build's `prebuild:plugin` would otherwise regenerate these files and the check would compare them against themselves), so committed drift fails the build. Because `--check` runs pre-build, the codegen reaches gate-backend sources whose `@transcodes-guard/core/*` imports resolve against the **committed** core dist — definition modules must therefore only use long-existing core runtime exports (new core exports are safe to *type*-import only).
