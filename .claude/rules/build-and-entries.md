...
- `build:plugin` runs a `prebuild:plugin` step (`scripts/stamp-build-info.mjs`) that **generates committed source** from root `package.json` version: `packages/mcp-server-core/src/build-info.ts` and `plugins/*/src/version.ts`. These are committed source (not gitignored) and hand-edits are overwritten each build.
