...
- **Contributors are not required to rebuild `dist/` on feature branches.** `release.yml` rebuilds and commits `dist/` on the Release PR, so the release **tag** (the actual deploy unit) is always fresh; stale `dist/` on a mid-cycle `main` is harmless because hosts install from a tag. Rebuild (`npm run build:plugin`) only to verify locally — CI rebuilds fresh on every push regardless of what you committed.
...
- `build:plugin` runs a `prebuild:plugin` step that **generates committed source**: `scripts/stamp-build-info.mjs` stamps the version into `packages/mcp-server-core/src/build-info.ts` + `plugins/*/src/version.ts`, and `scripts/generate-router-files.mjs` renders the `/transcodes` body from the single source `scripts/router-body.mjs` into `packages/mcp-server-core/src/router-body.ts` + the four per-host command/skill files. All are committed source (not gitignored); hand-edits are overwritten each build, and CI's `--check` gate fails on router drift.
...
- Freshness is guarded only by: (a) `build:plugin` succeeding, (b) the **gitignore-existence guard**, (c) the **23 smoke tests** (claude-code 9 + codex 3 + antigravity 5 + cursor 6).
