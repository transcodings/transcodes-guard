...
- **Prompt hooks are telemetry-only and fail-soft.** Claude/Codex remain silent, Cursor always returns `{continue:true}`, and Antigravity recovers the prompt through PreInvocation. They cache current-turn text locally; they never poll, acknowledge step-up, call the backend, or block a prompt. PreToolUse prefers an exact prompt/turn id cache hit, otherwise the latest session prompt when the host has no turn id, then falls back to transcript.
