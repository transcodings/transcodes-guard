...
- **Prompt hooks are telemetry-only and fail-soft.** Claude/Codex remain silent, Cursor always returns `{continue:true}`, and Antigravity recovers the prompt through PreInvocation. They cache current-turn text locally; they never poll, acknowledge step-up, call the backend, or block a prompt. PreToolUse reads the cache only on an exact prompt/turn id match — a cache entry is safe only when both hook events carry the same opaque turn id, so a host that sends no turn id in PreToolUse (Antigravity) never reads the cache and goes straight to the transcript fallback. There is no latest-entry fallback: it could attach a previous user turn to the current tool call.
...
- No `SessionStart`, no `UserPromptSubmit` events. **PreInvocation** fires before every model call: primer + no-token notice run only at the official 0-indexed `invocationNum === 0`; prompt capture tails the latest user message on every invocation and retries at 25/75 ms on invocation zero to tolerate transcript lag. The adapter's `emitSessionStartContext`/`emitUserPromptSubmitContext`/`parseUserPromptSubmitStdin` intentionally **throw** to surface wiring bugs.
